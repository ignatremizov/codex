...
This document describes how to measure adaptive stream chunking and anti-flap
behavior. The historical observations below are not evidence that the current
revision has been exercised.
...
With an already-built Codex executable, enable chunking traces:
...
For a local development build, `just codex` requires explicit authorization to
compile and run locally. Do not enable the removed `responses_websockets` feature;
the chunking policy is transport-independent.
...
These are historical observations from earlier tuning passes, retained to
explain the selected policy. They are not fresh measurements of this revision.
