...
Expose the optional start and deadline timestamps on `CommandExecutionRequestApprovalParams`.
Both wire fields remain optional for older app-server compatibility; if either is absent, the
approval is untimed. Follow app-server v2 timestamp conventions when choosing wire names and units.
