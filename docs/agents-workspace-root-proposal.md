...
The marker is not implemented by this document. Current discovery continues to use the existing project-root rules until a separate implementation lands.
...
All probes and reads remain subject to the selected environment's existing filesystem sandbox and trust checks. Remote discovery must retain the owning environment and use its `ExecutorFileSystem` with `PathUri` ancestry; a marker must not turn an executor-owned URI into a host path or bypass the current refusal to load project instructions for an untrusted project.
...
- Foreign executor operating systems retain their own path convention, sandbox checks, and environment provenance.
