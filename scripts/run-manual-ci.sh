...
Build while the rest of Verify continues. The script monitors both workflows
until they succeed and retries Verify when GitHub Actions fails before running
repository steps.
...
while true; do
  if ! verify_json="$(
    ...
    gh run view "$build_run" --repo "$repo" --json status,conclusion,url 2>/dev/null
  )"; then
  ...
  pair="Verify=$verify_status/$verify_conclusion Build=$build_status/$build_conclusion"
  ...
  if [[ "$verify_status" == "completed" && "$build_status" == "completed" ]]; then
    echo "Verify and Build succeeded."
