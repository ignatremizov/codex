...
while true; do
  ...
  while true; do
    ...
    # The reusable static job can skip Clippy only after restoring a successful
    # same-SHA/full-Clippy marker. Accept that explicit reuse step, not any skip.
    static_reused="$(jq -r '[.jobs[].steps[] | select(.name == "Reuse successful static checks" and .conclusion == "success")] | length > 0' <<<"$verify_json")"
    if [[ "$static_reused" == true || ( "$clippy_status" == "completed" && "$clippy_conclusion" == "success" ) ]]; then
      break
    fi
