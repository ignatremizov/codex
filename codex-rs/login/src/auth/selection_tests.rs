...
#[tokio::test]
#[serial(codex_auth_env)]
async fn guarded_chatgpt_session_reloads_only_its_selected_source() -> std::io::Result<()> {
    let home = tempfile::tempdir()?;
    let selected = config(home.path(), "auth-office.json")?;
    let initial = credentials("office");
    save_auth_for_selection(
    ...
    let session = ChatgptAuthSession {
        inner,
        expected_account_id: Some("office".to_string()),
    };
    save_auth(
        home.path(),
        &credentials("unrelated-default"),
        AuthCredentialsStoreMode::File,
        AuthKeyringBackendKind::Direct,
    )?;
    assert_eq!(
    ...
    rotated
        .tokens
        .as_mut()
        .expect("managed tokens")
        .access_token = "rotated-office".to_string();
    save_auth_for_selection(
        home.path(),
        &selected.auth_file_selection,
        &rotated,
    ...
    save_auth_for_selection(
        home.path(),
        &selected.auth_file_selection,
        &credentials("different-office-account"),
        selected.auth_credentials_store_mode,
        selected.keyring_backend_kind,
    )?;
