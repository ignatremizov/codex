...
fn has_uuid_v7(value: &str) -> bool {
    uuid::Uuid::parse_str(value).ok().is_some_and(|uuid| {
...
pub fn ordinary_agent_message_response_item_id(id: &str) -> String {
    if sub_agent_completion_status_from_response_item_id(id).is_some()
        ...
        || is_user_agent_task_context_response_item_id(id)
...
impl CollabAgentToolCallItem {
    /// Whether this terminal wait explicitly owns presentation of terminal child statuses.
    pub fn owns_completion_presentation(&self) -> bool {
        ...
        self.tool == CollabAgentTool::Wait
            ...
            && agent_ids.iter().all(|id| {
                self.agents_states
                    .get(id)
                    .and_then(SubAgentCompletionStatus::from_agent_status)
                    .is_some()
            })
    }
}
