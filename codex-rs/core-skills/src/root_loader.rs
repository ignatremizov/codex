...
fn merge_skill_root_snapshots(snapshots: Vec<SkillRootSnapshot>) -> SkillLoadOutcome {
    ...
    let mut display_path_by_skill_path = HashMap::new();
    ...
    for snapshot in snapshots {
        let SkillRootSnapshot {
            ...
            skills,
            errors,
            display_path_by_skill_path: snapshot_display_paths,
            file_system,
        } = snapshot;
        ...
        for (path, display_path) in snapshot_display_paths {
            display_path_by_skill_path
                .entry(path)
                .or_insert(display_path);
        }
    ...
    display_path_by_skill_path.retain(|path, _| retained_skill_paths.contains(path));
    outcome.skill_roots = skill_roots;
    outcome.skill_root_by_path = Arc::new(skill_root_by_path);
    outcome.display_path_by_skill_path = Arc::new(display_path_by_skill_path);
