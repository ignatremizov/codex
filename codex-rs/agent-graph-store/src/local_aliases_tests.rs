...
#[tokio::test]
async fn deleted_aliases_remain_reserved_without_authorizing_revival() {
    let fixture = state_runtime().await;
    let store = LocalAgentGraphStore::new(Arc::clone(&fixture.state_db));
    let root = thread_id(/*suffix*/ 800);
    let child = thread_id(/*suffix*/ 801);
    let destination = thread_id(/*suffix*/ 802);
    ...
    let root_alias = store
        ...
        .await
        .unwrap();
    // An unpublished root has no thread row or incoming edge, but remains a valid namespace.
    assert_eq!(
        store
    ...
    let child_alias = store
        ...
        .await
        .unwrap();
    fixture.state_db.delete_thread(child).await.unwrap();
    assert_eq!(
        store.list_agent_aliases(session_id).await.unwrap(),
        vec![
            root_alias,
            AgentAlias {
                state: AgentAliasState::Closed,
    ...
    store
        .transfer_agent_alias(TransferAgentAliasRequest {
            expected_previous_session_id: None,
            expected_descendant_thread_ids: Vec::new(),
        ...
        .await
        .expect_err("adoption must not revive a deleted UUID");
    store
        .allocate_agent_alias(AllocateAgentAliasRequest {
            session_id,
            parent_thread_id: root,
    ...
    let next = store
        ...
        .await
        .unwrap();
    assert_eq!(next.agent_ref, 3);

    fixture
        .state_db
        .delete_threads_strict(&[root, next.thread_id])
        .await
        .unwrap();
    let root_tombstone = store
        .ensure_agent_alias_namespace(session_id)
        .await
        .unwrap();
    assert_eq!(root_tombstone.state, AgentAliasState::Closed);
    assert_eq!(
        store
            .find_current_agent_alias_by_thread(root)
            .await
