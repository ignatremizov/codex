...
*** End Patch"#
    ...
    #[tokio::test]
    async fn repeated_update_sections_apply_against_prior_update_contents() {
        let session_dir = tempdir().unwrap();
        let relative_path = "source.txt";
        let original = "one\ntwo\n";
        fs::write(session_dir.path().join(relative_path), original).unwrap();
        let argv = vec![
            "apply_patch".to_string(),
            r#"*** Begin Patch
*** Update File: source.txt
...
*** End Patch"#
        ...
        assert_eq!(
            fs::read_to_string(session_dir.path().join(relative_path)).unwrap(),
            original
        );
    }

    #[tokio::test]
    async fn repeated_update_sections_preserve_crlf_line_endings() {
        let session_dir = tempdir().unwrap();
        let relative_path = "source.txt";
        let original = "one\r\ntwo\r\n";
        fs::write(session_dir.path().join(relative_path), original).unwrap();
...
*** End Patch"#
        ...
        let MaybeApplyPatchVerified::Body(action) = maybe_parse_apply_patch_verified_with_mode(
            &argv,
            &PathUri::from_host_native_path(session_dir.path()).expect("absolute test path"),
        ...
        assert_eq!(
            ...
            Some(&ApplyPatchFileChange::Update {
                ...
                move_path: None,
                new_content: expected_content.to_string(),
            })
        );
        assert_eq!(
            fs::read_to_string(session_dir.path().join(relative_path)).unwrap(),
    ...
    #[tokio::test]
    async fn duplicate_operations_keep_existing_restrictions() {
        let cases = [
            (
                "update_then_add",
                "before\n",
        ...
        for (name, original, operations) in cases {
            let session_dir = tempdir().unwrap();
            ...
            assert!(
                matches!(
                    ...
                    MaybeApplyPatchVerified::CorrectnessError(
                        ApplyPatchError::ParseError(ParseError::InvalidPatchError(message))
                    ) if message.contains("multiple operations target")
                ),
                "case {name} should reject duplicate operations: {result:?}"
            );
            if original.is_empty() {
                assert!(
                    !source_path.exists(),
                    "case {name} must not create the source"
                );
            } else {
                assert_eq!(
                    fs::read_to_string(&source_path).unwrap(),
                    original,
                    "case {name} must not mutate the source"
                );
            }
            assert!(!session_dir.path().join("renamed.txt").exists());
        }
    }
