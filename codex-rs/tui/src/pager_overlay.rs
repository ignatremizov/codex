...
struct CellRenderable {
    ...
    layout_revision: std::cell::Cell<u64>,
...
impl CellRenderable {
    fn new(cell: Arc<dyn HistoryCell>, style: Style, detail_mode: TranscriptDetailMode) -> Self {
        ...
        Self {
            ...
            detail_mode,
            has_stable_layout,
            layout_revision: std::cell::Cell::new(0),
...
impl Renderable for CellRenderable {
    ...
    fn layout_revision(&self) -> Option<u64> {
        if !self.has_stable_layout {
            let revision = self.layout_revision.get().wrapping_add(1);
            self.layout_revision.set(revision);
            Some(revision)
        } else {
            self.cell.transcript_animation_tick()
        }
    }
}
