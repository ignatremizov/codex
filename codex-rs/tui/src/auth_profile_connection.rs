...
pub(crate) async fn connect(
    ...
    let AppServerClient::Remote(mut client) =
...
pub(crate) async fn connect_for_startup(
    ...
    match target {
        ...
        AppServerTarget::LocalDaemon { .. } => match connect(target, config).await {
            Ok(client) => Ok(Some(client)),
            Err(_) => {
                tracing::debug!("local app-server profile unavailable; using embedded runtime");
                Ok(None)
            }
        },
    }
