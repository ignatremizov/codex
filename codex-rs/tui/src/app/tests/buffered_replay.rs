...
fn buffered_replay_keeps_unfinished_and_unrelated_deltas_in_order() {
    ...
    let actual = events
        ...
        .collect::<Vec<_>>();
    assert_eq!(
        serde_json::to_value(actual).expect("serialize actual events"),
        ...
        .expect("serialize expected events")
    );
...
async fn misalignment_buffered_replay_preserves_input_after_continuation() {
    ...
    let rendered = std::iter::from_fn(|| history.try_recv().ok())
        ...
        .collect::<String>();
    insta::assert_snapshot!(rendered, @"• Called sample.inspect({})
  └ Error: interrupted
");
    ...
    // Cover the whole stream, an evicted completion followed by a new delta, and a
    // post-completion notice after all lifecycle notifications were evicted.
...
#[tokio::test]
async fn misalignment_replay_blocks_when_latest_turn_start_was_evicted() {
    let thread_id = ThreadId::new();
    let error = AppServerTurnError {
        misalignment: None,
        message: "Chat stopped".into(),
