...
#[tokio::test]
async fn replay_only_thread_resume_failure_is_shown_without_exiting() -> Result<()> {
    let (mut app, mut app_event_rx, mut op_rx) = make_test_app_with_channels().await;
    let mut tui = crate::tui::test_support::make_test_tui()?;
    let mut app_server = Box::pin(crate::start_embedded_app_server_for_picker(&app.config)).await?;
    ...
    ■ Failed to resume agent thread: thread/resume failed during TUI bootstrap: thread/resume failed: no rollout found for thread id 123e4567-e89b-12d3-a456-426614174001 (code -32600)
    ");
...
#[tokio::test]
async fn replay_only_compact_resume_failure_is_shown_without_exiting() -> Result<()> {
    let (mut app, mut app_event_rx, _op_rx) = make_test_app_with_channels().await;
    let mut tui = crate::tui::test_support::make_test_tui()?;
    let mut app_server = Box::pin(crate::start_embedded_app_server_for_picker(&app.config)).await?;
    let thread_id = ThreadId::from_string("123e4567-e89b-12d3-a456-426614174002")?;
    app.active_thread_id = Some(thread_id);
    app.ensure_thread_channel(thread_id).mark_replay_only();
    app.chat_widget
        .handle_thread_session(test_thread_session(thread_id, app.config.cwd.to_path_buf()));
