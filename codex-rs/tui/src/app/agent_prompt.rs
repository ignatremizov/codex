...
impl App {
    ...
    pub(super) async fn submit_agent_prompt_to_selector(
        ...
        let response = super::agent_prompt_queue::response_handling_option(response_handling)
            ...
            .unwrap_or_default();
        let recovery_command = format!("/agent {}{response}", selector.authored());
        match selector.control_target() {
            Ok(_) => {}
            Err(message) => {
        ...
        let recovery_command = format!("/agent {thread_id}{response} ");
        let outcome = self
            ...
            .await;
        if outcome == AgentPromptSubmission::Rejected {
            self.recover_agent_command(source_thread_id, recovery_command, user_message);
        }
    }
    ...
    pub(super) async fn submit_reserved_agent_prompt(
        ...
        input: Vec<UserInput>,
    ) -> Result<(Option<String>, AgentInputOutcome)> {
        self.ensure_agent_control_admission(source_thread_id, Some(target_thread_id))?;
        ...
        // another future turn.
        if input_outcome != AgentInputOutcome::Queued {
            self.agent_navigation
                .clear_reserved_prompt_response(target_thread_id);
        }
        if input_outcome == AgentInputOutcome::Admitted {
            self.agent_navigation.mark_running(target_thread_id);
        }
        if input_outcome == AgentInputOutcome::Unknown || post_admission_warning.is_some() {
            self.agent_navigation
        ...
        if input_outcome == AgentInputOutcome::Unknown {
            self.chat_widget.add_error_message(format!(
                "Input outcome for agent {target_thread_id} is unknown. Reload its transcript before \
                 retrying; the authored input is retained for reconciliation."
            ));
        }
        if let Some(warning) = post_admission_warning {
            self.chat_widget.add_error_message(format!(
                "Agent {target_thread_id} input warning; do not retry automatically: {warning}"
            ));
        }
        if let Some(warning) = audit_warning {
            self.chat_widget.add_error_message(format!(
                "The agent input outcome could not be audited; do not retry automatically: {warning}"
            ));
        }
        Ok((turn_id, input_outcome))
    }
    ...
    pub(super) async fn submit_agent_prompt_with_control(
        ...
        if let Err(error) = self.ensure_agent_control_admission(source_thread_id, Some(thread_id)) {
            ...
            .await
        {
            Ok(outcome @ AgentPromptSubmission::Admitted { .. }) => {
                ...
                if *input_outcome == AgentInputOutcome::Unknown {
                    let response =
                        super::agent_prompt_queue::response_handling_option(response_handling)
                            .map(|option| format!(" {option}"))
                            .unwrap_or_default();
                    let queue = match admission {
                        AgentPromptAdmission::Direct => "",
                        AgentPromptAdmission::Queued => " queue",
                    };
                    self.recover_agent_command(
                        source_thread_id,
                        format!("/agent{queue} {thread_id}{response} "),
                        user_message,
                    );
                    self.chat_widget.add_error_message(format!(
                        "Input outcome for {label} is unknown. Reload the transcript before retrying; \
                         the authored input is retained for reconciliation."
                    ));
                }
                if *input_outcome == AgentInputOutcome::Queued
                    && admission == AgentPromptAdmission::Direct
                ...
                if let Some(warning) = audit_warning {
                    self.chat_widget.add_error_message(format!(
                        "Input for {label} could not be audited; do not retry automatically: {warning}"
                    ));
                }
                if let Some(warning) = &post_admission_warning {
                    self.chat_widget.add_error_message(format!(
                        "Input for {label} has a delivery warning; do not retry automatically: {warning}"
                    ));
                }
                outcome
            }
    ...
    async fn submit_agent_prompt_items(
        ...
        self.ensure_agent_control_admission(source_thread_id, Some(thread_id))?;
