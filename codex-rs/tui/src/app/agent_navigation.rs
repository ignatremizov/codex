...
mod regression_tests {
    ...
    fn upsert_preserves_known_identity_when_update_omits_metadata() {
        ...
        state.mark_subagent(thread_id);
        state.set_agent_path(thread_id, Some("/root/reviewer".into()));
        ...
        assert_eq!(
            ...
            Some(&AgentPickerThreadEntry {
                ...
                agent_path: Some("/root/reviewer".into()),
                is_running: false,
                is_closed: false,
            })
        );
        assert!(state.is_subagent(thread_id));
    }
