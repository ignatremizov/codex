...
impl App {
    pub(super) async fn queue_agent_prompt_to_selector(
        ...
        let response = response_handling_option(response_handling)
            .map(|option| format!(" {option}"))
            .unwrap_or_default();
        let recovery_command = format!("/agent queue {}{response}", selector.authored());
            ...
            .map(|row| {
                ...
                SelectionItem {
                    ...
                    secondary_action: Some(crate::bottom_pane::SelectionSecondaryAction {
                        ...
                        footer_hint: "Tab opens actions".into(),
                    }),
                    actions: vec![Box::new(move |tx| {
                        tx.send(AppEvent::EditQueuedAgentPrompt {
                            target_thread_id,
