...
impl App {
    ...
    pub(super) fn cache_collab_response_observation_for_notification(
        ...
        let observer = ThreadId::from_string(sender_thread_id)
            .ok()
            .filter(|observer| observer.to_string() == notification.thread_id);
        let response_handling = (observe_commentary.is_some()
        ...
        .then(|| {
            ...
            AgentResponseHandling::new(
                observe_commentary.unwrap_or(false),
                final_response,
                target_messages.unwrap_or(false),
                /*queue_input*/ false,
            )
        });
        let records_observation = response_handling.is_some()
        ...
        for receiver_thread_id in receiver_thread_ids {
            ...
            // A tool result can describe an older target turn. Only the target's own
            // matched lifecycle notification may retire its current running state.
        }
    }
}
