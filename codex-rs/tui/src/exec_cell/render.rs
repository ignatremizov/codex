...
mod tests {
    ...
    #[test]
    fn command_output_limit_counts_logical_lines_not_wrapped_rows() {
        let output = (1..=12)
            .map(|idx| {
                format!(
        ...
        assert!(
            ...
            "expected wrapped rows under the logical line limit to remain untruncated, got:\n{rendered_text}"
        );
        assert!(
            (1..=12).all(|idx| rendered_text.contains(&format!("line-{idx:02}"))),
            "expected every logical output line to remain visible, got:\n{rendered_text}"
        );
    }
