...
async fn live_app_server_inter_agent_final_answer_renders_in_transcript() {
    ...
    chat.handle_server_notification(
        ServerNotification::ItemCompleted(ItemCompletedNotification {
            ...
            item: AppServerThreadItem::AgentMessage {
                id: "item-stable".to_string(),
                text: "Agent final answer from `/root/direct_input_demo`:\n\nLorem ipsum dolor sit amet."
                    .to_string(),
                phase: Some(MessagePhase::Commentary),
                memory_citation: None,
                delivery: None,
...
#[tokio::test]
async fn live_app_server_context_compaction_item_completed_renders_prompt() {
    let (mut chat, mut rx, _ops) = make_chatwidget_manual(None).await;
...
#[tokio::test]
async fn live_app_server_context_compaction_decode_error_renders_when_summary_is_hidden() {
    let (mut chat, mut rx, _ops) = make_chatwidget_manual(None).await;
    chat.config.show_compact_summary = false;
    ...
    let cells = drain_insert_history(&mut rx);
    assert_eq!(cells.len(), 1);
    let rendered = lines_to_single_string(cells.last().expect("compaction error cell"));
    insta::assert_snapshot!(
        rendered,
        @r"
    ■ Compacted prompt decoding failed: Selected model is at capacity.
    "
    );
}

#[tokio::test]
async fn live_app_server_context_compacted_notification_renders_after_item() {
    let (mut chat, mut rx, _ops) = make_chatwidget_manual(None).await;
    ...
    assert!(
        rendered_cells
            .iter()
            .all(|rendered| rendered.contains("Compacted prompt"))
    );
    assert!(
        rendered_cells
            .iter()
            .all(|rendered| rendered.contains("Prompt line 1"))
    );
    assert!(
        rendered_cells
            .iter()
            .all(|rendered| rendered.contains("Prompt line 2"))
    );
}

#[tokio::test]
async fn live_app_server_remote_context_compacted_notification_renders_summary() {
    let (mut chat, mut rx, _ops) = make_chatwidget_manual(None).await;
    ...
    assert!(rendered.contains("Remote compact summary"));
}
