...
impl ChatWidget {
    ...
    /// Clears process-local terminal tracking without removing rendered command history.
    pub(crate) fn clear_unified_exec_process_tracking(&mut self) {
        self.unified_exec_processes.clear();
        self.completed_unified_exec_processes.clear();
        self.sync_unified_exec_footer();
    }

    /// Finalizes replayed command starts as audit history and discards process-local tracking.
