...
impl App {
    pub(super) fn attach_committed_user_message_source(
        &mut self,
        turn_id: &str,
        item_id: &str,
        ...
        if !Self::attach_source_to_latest_matching_user_cell(
    ...
    fn attach_source_to_latest_matching_user_cell(
        cells: &mut [Arc<dyn crate::history_cell::HistoryCell>],
        content: &[AppServerUserInput],
        source: UserMessageSource,
    ) -> bool {
        let display = ChatWidget::user_message_display_from_inputs(content);
        let Some((index, cell)) = cells.iter().enumerate().rev().find_map(|(index, cell)| {
            ...
            (cell.source.is_none()
                ...
                .then_some((index, cell))
        }) else {
            return false;
        };
        let mut cell = cell.clone();
        cell.source = Some(source);
        cells[index] = Arc::new(cell);
        true
    }
...
pub(crate) fn backtrack_fork_before_turn_id_with_source(
    turns: &[Turn],
    nth_user_message: usize,
...
pub(crate) fn backtrack_rollback_target_with_source(
    turns: &[Turn],
    nth_user_message: usize,
    prompt_occurrences: usize,
    prompt_occurrence: usize,
...
fn backtrack_prompt_turn_index(
    ...
    if let Some(source) = source {
        return backtrack_prompt_turn_index_by_source(turns, source, prompt);
    }
...
fn backtrack_prompt_turn_index_by_source(
    turns: &[Turn],
    source: &UserMessageSource,
    prompt: &mut UserMessage,
) -> Result<usize> {
    let Some((turn_index, turn)) = turns
        .iter()
        .enumerate()
        .find(|(_, turn)| turn.id == source.turn_id)
    else {
        bail!("the selected prompt turn was not found in the persisted thread");
    };
    let Some((item_index, content)) = turn.items.iter().enumerate().find_map(|(index, item)| {
        let ThreadItem::UserMessage { id, content, .. } = item else {
            return None;
        };
        (id == &source.item_id).then_some((index, content))
    }) else {
        bail!("the selected prompt item was not found in the persisted thread");
    };
...
mod tests {
    ...
    fn backtrack_fork_before_turn_id(
        turns: &[Turn],
        nth_user_message: usize,
        prompt_occurrences: usize,
        prompt_occurrence: usize,
        ...
        backtrack_fork_before_turn_id_with_source(
            turns,
            nth_user_message,
            prompt_occurrences,
            prompt_occurrence,
    ...
    fn backtrack_rollback_target(
        ...
        backtrack_rollback_target_with_source(
            turns,
            nth_user_message,
            prompt_occurrences,
            prompt_occurrence,
    ...
    #[test]
    fn backtrack_prefers_canonical_source_over_rendered_prompt_projection() {
        let turns = vec![
            turn("turn-1", TurnStatus::Completed, /*user_messages*/ 1),
            turn("turn-2", TurnStatus::Completed, /*user_messages*/ 1),
        ];
        let source = UserMessageSource {
            item_id: "user-0".to_string(),
        ...
        assert_eq!(
            backtrack_rollback_target_with_source(
                ...
                /*prompt_occurrence*/ 0,
                Some(&source),
                &mut prompt("stale rendered prompt"),
            )
            .expect("canonical source should resolve without rendered-text rediscovery"),
            rollback_target(
    ...
    #[test]
    fn committed_source_attaches_to_existing_optimistic_prompt() {
        let mut cells = vec![Arc::new(UserHistoryCell {
        ...
        assert!(App::attach_source_to_latest_matching_user_cell(
            &mut cells,
            &[UserInput::Text {
