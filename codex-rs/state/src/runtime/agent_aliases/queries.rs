...
SELECT
    ...
    EXISTS (SELECT 1 FROM agent_alias_tombstones AS tombstone
            WHERE tombstone.thread_id = alias.thread_id) AS deleted
...
WHERE alias.thread_id = ?
  ...
  AND NOT EXISTS (
      SELECT 1 FROM agent_alias_tombstones WHERE thread_id = alias.thread_id
  )
            "#,
        )
        .bind(thread_id.to_string())
...
pub(super) async fn require_active_parent_alias(
    ...
    super::require_not_deleted(tx, parent_thread_id).await?;
...
fn agent_alias_from_row(row: SqliteRow) -> anyhow::Result<crate::AgentAliasRecord> {
    ...
    let state = match ownership_state {
        ...
        AgentAliasOwnershipState::Current if row.try_get::<i64, _>("deleted")? != 0 => {
