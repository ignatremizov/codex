...
use crate::AgentLifecycleAuthorityUpdate;
...
async fn lifecycle_authority_epochs_advance_only_on_open_to_closed_transitions() {
    ...
    assert_eq!(
        ...
        AgentLifecycleAuthorityUpdate::NotOwned,
    );
    assert_eq!(
        runtime
            .read_agent_thread_lifecycle_epochs(&[target, child])
    ...
    assert_eq!(
        runtime
            .read_agent_thread_lifecycle_epochs(&[target, child])
            .await
            .expect("incomplete revocation rolls back"),
        vec![(target, 0), (child, 0)],
    );
    for expected in [
        AgentLifecycleAuthorityUpdate::Revoked,
        AgentLifecycleAuthorityUpdate::Unchanged,
    ] {
        assert_eq!(
            runtime
                .set_agent_lifecycle_state_with_authority_revocations(
