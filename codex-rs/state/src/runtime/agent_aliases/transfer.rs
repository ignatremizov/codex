...
impl StateRuntime {
    ...
    pub async fn transfer_agent_alias(
        ...
        super::require_not_deleted(&mut tx, thread_id).await?;
        super::require_not_deleted(&mut tx, new_parent_thread_id).await?;
...
ORDER BY subtree.thread_id
    ...
    for row in rows {
        ...
        super::require_not_deleted(tx, descendant_thread_id).await?;
