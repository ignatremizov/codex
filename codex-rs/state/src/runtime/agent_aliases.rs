...
pub(super) async fn require_not_deleted(
    ...
    if sqlx::query_scalar::<_, i64>("SELECT 1 FROM agent_alias_tombstones WHERE thread_id = ?")
        .bind(thread_id.to_string())
        .fetch_optional(&mut **tx)
        .await?
        .is_some()
    {
        anyhow::bail!("agent {thread_id} was permanently deleted");
    }
    Ok(())
}
...
impl StateRuntime {
    ...
    pub async fn reserve_agent_aliases_for_fork(
        ...
        let mut tx = self.pool.begin_with("BEGIN IMMEDIATE").await?;
        require_not_deleted(&mut tx, source_session_id.into()).await?;
        require_not_deleted(&mut tx, fork_session_id.into()).await?;
...
WHERE session_id = ?
  AND NOT EXISTS (
      SELECT 1 FROM agent_alias_tombstones
      WHERE thread_id = agent_alias_namespaces.session_id
  )
  AND NOT EXISTS (
      SELECT 1
      FROM agent_aliases
  ...
  AND NOT EXISTS (
    ...
    pub async fn set_agent_lifecycle_state(
        ...
        let mut tx = self.pool.begin_with("BEGIN IMMEDIATE").await?;
        require_not_deleted(&mut tx, thread_id).await?;
