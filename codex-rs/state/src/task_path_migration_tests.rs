...
async fn task_path_migration_upgrades_existing_aliases_without_reusing_lifecycle_paths() {
    ...
    // Exercise the current released schema, including the later tombstone migration, without
    // changing any released migration bytes when inserting the task-label migration.
    let mut released = migrator_through(/*version*/ 10_054);
    released
        .migrations
        .to_mut()
        .retain(|migration| migration.version != 10_051);
    released
        .run(&mut connection)
        .await
