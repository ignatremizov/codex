...
async fn goal_service_external_set_active_preserves_concurrent_usage() -> anyhow::Result<()> {
    ...
    let active_goal_objective = harness
        ...
        .get::<ActiveGoalObjective>()
        .expect("active goal objective state");
    active_goal_objective.replace(Some("unrelated objective".to_string()));
...
async fn post_compaction_context_reconstructs_current_active_goal() -> anyhow::Result<()> {
    ...
    let items = harness.post_compaction_context_items().await;
...
impl GoalExtensionHarness {
    ...
    async fn post_compaction_context_items(&self) -> Vec<ResponseItem> {
        let mut items = Vec::new();
        for contributor in self.registry.context_contributors() {
            let mut contribution = contributor
                .contribute_post_compaction_context(&self.session_store, &self.thread_store)
                .await;
