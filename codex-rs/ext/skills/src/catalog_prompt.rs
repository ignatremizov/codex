...
const SKILLS_HOW_TO_USE_WITH_SOURCE_LOCATORS: &str = r###"- Trigger rules: Follow a skill's instructions if the user names it (with `$SkillName` or plain text) or the task clearly matches its description shown above.
...
const SKILLS_HOW_TO_USE_WITH_HOST_ALIASES: &str = r###"- Trigger rules: Follow a skill's instructions if the user names it (with `$SkillName` or plain text) or the task clearly matches its description shown above.
