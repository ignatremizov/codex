...
- unix socket (`--listen unix://` or `--listen unix://PATH`): websocket connections over the startup auth profile's socket under `$CODEX_HOME/app-server-control/`, or a custom socket path, using the standard HTTP Upgrade handshake
...
opens exactly one raw stream connection to the selected auth profile's control socket
...
- Begin a turn: To send user input, call `turn/start` with the target `threadId` and the user's input. Optional fields let you override model, cwd, sandbox policy or experimental `permissions` profile selection, approval policy, approvals reviewer, etc. The response identifies the turn that admitted the input: normally a new turn, or an intervening active turn when the input was steered into it. The app-server emits `turn/started` when a new turn actually begins running.
...
After initialization, `server/read` with `{}` returns
`{ "authProfile": { "profileOpaqueId": "...", "displayLabel": "codex-office" } }`.
This metadata describes the immutable startup credential selection, not the
authenticated account. It contains no credentials or absolute auth-file path.
The opaque identity includes the default storage policy, so a default keyring
profile is not interchangeable with an explicitly selected file. Friendly
labels are for display and must not be used as identity keys.

Local shared-server clients verify both the initialized `codexHome` and the
`server/read` profile on the actual connection before using it. A separate
socket probe is not proof of the server used by a later connection. Opaque
profile IDs are host-local and must not be compared across remote hosts;
remote endpoints own their authentication independently of the client's
local credential-file selection. Legacy remote servers may omit this method.
...
Agent-originated input is projected as `agentMessage` with optional `attribution` and
structured `input`. Attribution captures both `sender` and `recipient` identities and
`senderTurnId` at send time. Each identity contains the canonical `threadId`, nickname,
`agentRef`, `taskPath`, role, model, and reasoning effort. These display snapshots survive
alias changes and ownership transfers; they do not authorize routing. The original text,
text elements, and attachments remain in `input` without truncation. Clients should render
that input when attribution is present; `text` may be empty. Older messages without this
metadata retain their existing rendering. This presentation does not change Responses API
transport or item-ID prefixes, and human-authored spawn, prompt, and queued input remains
`userMessage` in the target thread.

`agent/control` spawn accepts optional `task`, resolved relative to the issuing agent's
assignment within the root namespace. Resume accepts `task` only when adopting an external
thread into that namespace; same-root resume cannot rename an assignment. Spawn and resume
outcomes expose the committed `taskPath`. Resume also returns `taskPathMapping`, a collection
of `{threadId, previousTaskPath, taskPath}` records for adoption. Both path fields are nullable
to represent assignment or removal of a label. User-control audit items keep the authored
...
Both endpoints must be live in the source's controlled graph; enabling neither resumes nor
adopts them. The `replyRouteChanged` outcome returns `targetThreadId` and
`recipientThreadId`, and its `userAgentControl` audit records `replyRecipientThreadId`.
Disabling affects only that sender→recipient route; enabling the reverse route is separate.
The permission lasts across live turns until disabled or its lifecycle ends, not across cold
resume or history fork.

`{"type":"subtreeMessaging","mode":"enabled"}` sets a live default for `sourceThreadId`
and its current/future descendants. Both endpoints must be in that subtree. Explicit `replyRoute`
settings override the nearest common supervisor's subtree default. Its `subtreeMessagingChanged`
outcome includes `rootThreadId`, `previousMode`, and `mode`; its durable `userAgentControl` audit
uses action `subtreeMessaging`, but replay never restores this live permission. Closing the
supervisor, changing ownership, cold resume, and history fork do not propagate the grant.
Enabling discovers peers through singleton route context without subscribing to final responses
or starting model turns. Stable and experimental JSON/TypeScript/precomputed exports must be
regenerated by CI together with these experimental protocol additions.

When neither endpoint is Main, accepted attributed input is also published as a live-only
`item/completed` agent-message presentation in the live root. Its trusted attributed-input
identity and `Agent message from \`<sender UUID>\` to \`<recipient UUID>\`:` header let
clients render both endpoints and `○ not visible`. The root copy is event-only: it creates
no rollout record, Responses input, steer, queued turn, or wake in Main. Queued input is copied only when
admitted; denied or cancelled queued input has no received-message audit. Full transcript
retains the complete text while normal history uses the configured preview. The TUI retains the
notification in its live replay buffer. Cold resume of Main does not reconstruct peer copies;
the communicating threads retain the canonical messages. Route-control decisions remain durable.

Protocol updates must include regenerated stable and experimental JSON/TypeScript schemas and
precomputed exports. When local compilation is prohibited, generate those artifacts in
Manual Verify CI and commit the reviewed output before shipping.

- `server/read` — read immutable startup auth-profile metadata after initialization.
...
- `thread/read` — read a stored thread by id without resuming it; optionally include turns via `includeTurns`. The returned `thread` includes `status` (`ThreadStatus`), defaulting to `notLoaded` when the thread is not currently loaded. Every loaded thread, including V1 and V2 spawned children, reports `canAcceptDirectInput: true`; unloaded stored threads report `null` until resumed.
...
- `agentAlias/list` — list durable root-scoped agent refs, nicknames, task paths, and lifecycle states in stable ref order (experimental; requires `capabilities.experimentalApi`); accepts `rootThreadId` naming the root or any member of its session tree, plus optional `cursor` and `limit`. An alias's nullable `taskPath` is its current assignment label, not a filesystem path or lifecycle edge.
- `agent/control` — perform a user-authored multi-agent operation relative to `sourceThreadId` (experimental; requires `capabilities.experimentalApi`). `spawn` creates a default or configured-role child, optionally admits its first structured user input, accepts `none`, `all`, or last-N history forking, and may override the resolved child `model` and `reasoningEffort`; explicit values take precedence over role and configured subagent defaults. `prompt` accepts a controlled target by ref, nickname, or UUID, automatically reopens a known same-root closed target, and preserves structured `UserInput`; the reserved nickname `Main` resolves the source root case-insensitively. `reservedPrompt` admits input using the next-turn response policy reserved by a prompt-less spawn or resume and returns the exact admitted `turnId`. `queuedPrompt` appends complete structured input to the process-lifetime, target-owned FIFO; it never steers an active turn, and an idle target starts the entry immediately. Its final response is queued for the source's next turn instead of steering active source work. `resume` reopens a controlled closed target or explicitly adopts a stored UUID; a live target or descendant owned by another root must be closed before adoption. Successful spawn and resume responses return the committed root-scoped `ref` and nickname when durable aliases are available, so clients need not wait for an alias-list refresh before presenting the identity. `interrupt` optionally admits a follow-up after cancellation. `close` ends a controlled runtime and can replay a completed response absent from effective source context passively, wake for it, queue it, or keep it presentation-only. `observe` authoritatively replaces passive, wake, or presentation-only final-response handling for a target's active, pending, or not-yet-delivered turn. `replyRoute` enables or disables a V1 target's attributed replies across later turns; enabling injects the source-relative route into target model context once, while disabling overrides model-authored `m`. Spawn, prompt, queued prompt, resume, interrupt follow-up, and close actions can independently select applicable response handling. Scoped reply routes currently require a target using the V1 adapter; unsupported targets reject `m` or `replyRoute` before input or queue mutation. Every accepted or rejected operation appends a durable `userAgentControl` item to the source thread with the authored selector, requested spawn model/reasoning values, canonical target when resolved, response policy, and outcome; a successful prompt sets `resumedTarget` when it reopened the target, and successful adoption also records the previous and new owner session IDs. When target input was admitted but its response policy could not be persisted, the committed outcome includes `postAdmissionWarning`; clients must retain the admitted action, clear any optimistic observation state, and must not retry it. If an exclusive adoption commits but subsequent resume setup fails, the `resumed` outcome includes `postCommitWarning`; ownership remains exclusive and the client may retry resume without repeating the transfer. Every committed operation returns its typed `outcome`; if the source audit write fails, the response also includes `auditWarning`, and clients must apply the outcome without retrying the operation.
- `agentQueue/list` — list pending process-lifetime agent turns for the root containing `rootThreadId` (experimental; requires `capabilities.experimentalApi`). Results retain structured input, source and target thread IDs, authored selector, prompt preview, and the response policy that will bind when the entry starts. The method uses cursor pagination. When an entry starts, the existing `turn/started` notification includes `agentQueue` with its entry ID and source thread ID; the queue entry's response handling is present only after its source-side state commits and combines with any response policy already reserved for that next turn. If later admission fails or response handling degrades after input admission, Core retains queue provenance, leaves the entry handling null, preserves any older next-turn policy on the admitted turn, and publishes a source-thread warning instead of silently retrying already-admitted input.
- `agentQueue/delete` — remove one pending entry from the same root-scoped queue by stable entry ID (experimental; requires `capabilities.experimentalApi`). Entries already admitted to a target turn are no longer pending and cannot be deleted through this method.
- `thread/rollback` — deprecated and will be removed soon. Drop the last N turns from the agent’s in-memory context and persist a rollback marker in the rollout so future resumes see the pruned history; returns the updated `thread` (with `turns` populated) on success. Clients that select the target from a prior `thread/read` can pass `expectedStartTurnId` and `expectedTurnCount` to reject a stale rollback target. If the rollback commits but the server cannot hydrate the response, the JSON-RPC error data includes `threadRollbackCommitted: true`. If the durability barrier fails after the marker becomes readable, or commit verification itself fails, the server unloads the ambiguous live thread before returning error data with `threadRollbackRefreshRequired: true`; clients must call `thread/resume` before deciding whether to retry. Paginated threads do not support rollback.
- `thread/revert` — experimental. Replace a loaded paginated thread's durable history with the prefix strictly before `beforeTurnId` while preserving its thread id and resolved multi-agent version across reloads. The operation interrupts an active turn if needed, leaves older rollout files immutable, reloads the thread, returns updated thread metadata with empty `turns` plus pagination cursors, and emits `thread/reverted`. It does not revert local file changes.
- `turn/start` — submit user input or a named standalone function-call output to a thread and begin Codex generation; responds with the admitted `turn` object and streams `turn/started`, `item/*`, and `turn/completed` notifications. If another writer starts a normal turn first, the response identifies that active turn after the input is steered into it; a non-steerable review or compaction turn rejects the request. For standalone outputs, provide `toolOutput` with an empty `input` array. Optional `turnTrigger` classifies who or what started a new turn and is sent as `turn_trigger` in Responses request metadata; it is ignored if the request steers an active turn. `clientUserMessageId` is optional; when supplied, the corresponding `userMessage` item echoes it as `clientId`. Experimental `runtimeWorkspaceRoots` supplies the default roots for newly resolved environment selections. Explicit `environments[].runtimeWorkspaceRoots` override that fallback with environment-native absolute paths. Prefer experimental `permissions` profile selection by id for permission overrides; the legacy `sandboxPolicy` field is still accepted but cannot be combined with `permissions`. For `collaborationMode`, `settings.developer_instructions: null` means "use built-in instructions for the selected mode". Deprecated experimental `multiAgentMode` is ignored; Ultra reasoning effort selects proactive behavior.
- `thread/inject_items` — append raw Responses API items to a loaded thread’s model-visible history without starting a turn; returns `{}` on success.
- `turn/settings/update` — experimental; publish a reviewer or model-settings patch to the exact live task identified by `threadId` and `turnId`, regardless of task kind. Model-settings updates require `step_model_switching`; reviewer-only updates do not. Returns `status: "applied"` or `status: "targetUnavailable"`, or a request error if rejected. Future-thread settings and already captured steps are unchanged.
...
Only one app-server process can hold a paginated thread open for writing at a time. If another process already owns the thread, `thread/resume`, `thread/archive`, and `thread/delete` fail with JSON-RPC error `-32600`. Archive also fails if another process owns any spawned descendant. Read-only requests remain available without resuming the thread.
...
- V1 `spawn_agent`, `send_input`, and `resume_agent` items include `observeCommentary` and
  `wakeOnCompletion` so clients can show whether that lifecycle call requested the target turn's
  first commentary response and an idle wake on completion. Other collab-agent tools report
...
Use `thread/delete` to hard-delete one thread. Spawn, adoption, and communication relationships do not cascade deletion; related threads and their durable graph relationships remain available. Existing rollout files and associated metadata for the selected thread must be removed before the request succeeds, and missing rollout files are treated as already deleted. Deletion is rejected when another paginated thread physically references the selected thread's history.
...
- `turn/started` — `{ turn, agentQueue }` with the turn id, empty `items`, and `status: "inProgress"`. `agentQueue` is `null` for ordinary turns; queued agent turns include their queue entry ID, source thread ID, and nullable committed queue-entry response handling.
...
- `collabAgentToolCall` — `{id, tool, status, observeCommentary, wakeOnCompletion, senderThreadId, receiverThreadIds, receiverAgents, prompt, model, reasoningEffort, agentsStates, inputBatch}` describing collab tool calls (`spawn_agent`, `send_input`, `resume_agent`, `wait`, `close_agent`); `status` is `inProgress`, `completed`, or `failed`. `observeCommentary` and `wakeOnCompletion` are `true` or `false` for V1 lifecycle calls and `null` for tools without V1 response observation. `inputBatch` is null for string-target sends and older history. Array-target sends carry `{flags, results}`: normalized shared flags and per-receiver `{target, receiverThreadId, status, error, hint}` results. Results are empty at start and populated on normal completion; status is `submitted`, `queued`, `mailboxAccepted`, or `error`. An error can follow admission, and cancellation may leave partial effects without a completed aggregate. Display receiver identities via `receiverAgents`, each outcome separately, and `prompt` once. Batch items do not emit a misleading single-receiver legacy interaction event.
...
- `sleep` — `{id, durationMs, outcome, elapsedMs}` emitted while the agent waits for a duration or new input. Started items have `null` completion metadata; completed items report an `outcome` of `completed`, `interrupted`, or `error` with the actual monotonic `elapsedMs`. Legacy history may omit the completion metadata.
