...
- `compacted` - `{threadId, turnId, summary?, message?}` when codex compacts the conversation history. `summary` contains the compacted text when available, and `message` contains the full compacted prompt stored in history. This can happen automatically.
