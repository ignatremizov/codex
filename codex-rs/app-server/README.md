...
- `mcpServerStatus/list` — enumerate configured MCP servers with their tools and auth status, plus resources/resource templates for `full` detail; supports optional `threadId` context plus cursor+limit pagination. If `detail` is omitted, the server defaults to `full`.
