...
#[tokio::test]
async fn plaintext_inter_agent_input_is_typed_once() -> Result<()> {
    skip_if_no_network!(Ok(()));
    run_inter_agent_input_delivery_case(MessageDeliveryCase::Plaintext).await
}

#[tokio::test]
async fn encrypted_inter_agent_input_is_typed_once() -> Result<()> {
    skip_if_no_network!(Ok(()));
    run_inter_agent_input_delivery_case(MessageDeliveryCase::Encrypted).await
}

#[tokio::test]
async fn encrypted_with_audit_inter_agent_input_is_typed_once() -> Result<()> {
    skip_if_no_network!(Ok(()));
    run_inter_agent_input_delivery_case(MessageDeliveryCase::EncryptedWithAudit).await
}

async fn run_inter_agent_input_delivery_case(message_delivery: MessageDeliveryCase) -> Result<()> {
...
async fn injected_agent_message_gets_a_stable_id_without_starting_model_work() -> Result<()> {
    ...
    let ThreadItem::AgentMessage {
        ...
        panic!("public inter-agent injection should preserve provenance");
    };
    assert_eq!(
    ...
    assert!(
        ...
        "history-only public injection must not start model work"
    );
...
async fn resume_rebuilds_legacy_inter_agent_communication_variants() -> Result<()> {
    ...
    assert_eq!(
        transcript_items
            .iter()
            .map(|(turn_id, _item_id, text, source)| (*turn_id, *text, *source))
            .collect::<Vec<_>>(),
        expected_transcript
    );
    let first_ids = transcript_items
        .iter()
        .map(|(turn_id, item_id, _, _)| (*turn_id, *item_id))
        .collect::<Vec<_>>();
    ...
    timeout(READ_TIMEOUT, app_server.shutdown_gracefully())
        .await
        .context("timed out shutting down warm replay server")??;
    let mut replay = TestAppServer::builder()
        .with_codex_home(codex_home.path())
        .build()
        .await?;
    timeout(READ_TIMEOUT, replay.initialize()).await??;
    let replay_request = replay
        ...
        .await?;
    let replay_response = timeout(
        READ_TIMEOUT,
        replay.read_stream_until_response_message(RequestId::Integer(replay_request)),
    )
    .await??;
    ...
    let replay_ids = replay_thread
        .turns
        .iter()
        .flat_map(|turn| {
            turn.items.iter().filter_map(|item| match item {
                ThreadItem::AgentMessage { id, .. } => Some((turn.id.as_str(), id.as_str())),
                _ => None,
            })
        })
        .filter(|(turn_id, _)| {
            expected_transcript
                .iter()
                .any(|(expected_turn_id, _, _)| expected_turn_id == turn_id)
        })
        .collect::<Vec<_>>();
...
async fn run_inter_agent_input_case(
    ...
    let (duplicate_started, duplicate_completed, duplicate_raw) =
        drain_pending_notifications_for_item(&mut app_server, id).await?;
    assert_eq!(
        duplicate_started, 0,
        "typed item/started must be emitted once"
    );
    assert_eq!(
        duplicate_completed, 0,
        "typed item/completed must be emitted once"
    );
    assert_eq!(duplicate_raw, 0, "raw item/completed must be emitted once");
...
async fn drain_pending_notifications_for_item(
    app_server: &mut TestAppServer,
    item_id: &str,
    ...
    timeout(READ_TIMEOUT, async {
        while !app_server.pending_notification_methods().is_empty() {
            let JSONRPCNotification { method, params } =
            ...
            match method.as_str() {
                "item/started" => {
                    let notification: ItemStartedNotification = serde_json::from_value(params)?;
                    if notification.item.id() == item_id {
                        started_count += 1;
                    }
                }
                "item/completed" => {
                    let notification: ItemCompletedNotification = serde_json::from_value(params)?;
                    if notification.item.id() == item_id {
                        completed_count += 1;
                    }
                }
                "rawResponseItem/completed" => {
                    ...
                    if notification
                        .item
                        .id()
                        .is_some_and(|id| id.as_str() == item_id)
                    {
                        raw_count += 1;
                    }
                }
        ...
        Ok::<_, anyhow::Error>((started_count, completed_count, raw_count))
    })
    .await
    .context("timed out draining pending inter-agent notifications")?
}
...
fn inter_agent_source_label(item: &ThreadItem) -> (&str, &str) {
    let ThreadItem::AgentMessage {
        inter_agent_source: Some(source),
        ..
    } = item
    else {
        panic!("inter-agent transcript item should carry provenance");
    };
    (&source.author, &source.recipient)
}
