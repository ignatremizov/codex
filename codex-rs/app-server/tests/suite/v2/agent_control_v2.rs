...
async fn queued_prompt_waits_for_idle_target_in_v1_and_v2() -> Result<()> {
    for multi_agent_v2 in [false, true] {
        for response_handling in [None, Some(AgentResponseHandling::Presentation)] {
...
async fn queued_prompt_waits_for_idle_target(
    multi_agent_v2: bool,
    ...
    let expected_handling = AgentResponseHandling {
        queue_input: true,
        ..response_handling.unwrap_or(AgentResponseHandling::new(
            /*commentary*/ false,
    ...
    .await;
    let source_reply = responses::mount_sse_once_match(
        &server,
        |request: &wiremock::Request| responses::body_contains(request, "queued turn finished"),
        responses::sse(vec![
            responses::ev_response_created("resp-user-control-queue-source"),
            responses::ev_assistant_message(
                "msg-user-control-queue-source",
                "queued completion received",
            ),
            responses::ev_completed("resp-user-control-queue-source"),
        ]),
    )
    .await;
    ...
    let queued: AgentControlResponse = app
        ...
        .await?;
    let outcome = agent_control_outcome(queued);
    let AgentControlOutcome::Prompted { submission_id, .. } = &outcome else {
    ...
    assert_eq!(
        ...
        AgentQueueListResponse {
            data: vec![AgentQueueEntry {
                ...
                response_handling: expected_handling,
                authored_selector: Some(target_thread_id.clone()),
            }],
            next_cursor: None,
        }
    ...
    timeout(DEFAULT_READ_TIMEOUT, async {
        while queued_turn.requests().is_empty() {
            tokio::task::yield_now().await;
        }
    })
    .await
    .context("queued child turn did not reach the Responses API")?;
    assert!(
        queued_turn
            .single_request()
            .body_contains_text(QUEUED_PROMPT)
    ...
    assert_eq!(
        ...
        TurnStartedNotification {
            ...
            agent_queue: Some(AgentQueueTurnMetadata {
                ...
                source_thread_id: root.thread.id.clone(),
                response_handling: Some(expected_handling),
            }),
        }
    );
    timeout(DEFAULT_READ_TIMEOUT, async {
    ...
    if response_handling.is_none() {
        timeout(DEFAULT_READ_TIMEOUT, async {
            while source_reply.requests().is_empty() {
                tokio::task::yield_now().await;
            }
        })
        .await
        .context("default queued completion did not reach the source model")?;
        assert!(
            source_reply
                .single_request()
                .body_contains_text("queued turn finished")
        );
    }
    Ok(())
}
