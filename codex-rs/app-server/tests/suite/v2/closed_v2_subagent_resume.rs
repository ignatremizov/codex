...
async fn write_v2_config(codex_home: &Path, server_uri: &str, max_threads: usize) -> Result<()> {
    std::fs::write(
...
async fn generic_resume_restores_closed_v2_subagent_through_live_owner(
    ...
    write_v2_config(codex_home.path(), &server.uri(), /*max_threads*/ 3).await?;
    ...
    write_v2_config(codex_home.path(), &server.uri(), /*max_threads*/ 2).await?;
    ...
    assert_eq!(followup_request.body_json()["model"], json!("gpt-5.4"));
    assert_eq!(
        followup_request.body_json()["reasoning"]["effort"],
        json!("high")
    );
    assert!(
        followup_request
            ...
            .iter()
            .any(|text| text == ROLE_INSTRUCTIONS),
        "restored worker request should retain the configured role instructions"
    );
    ...
    let completion_item = timeout(DEFAULT_TIMEOUT, async {
        loop {
            ...
            if completed.thread_id == root_thread.id
                ...
                && source.author == "/root/worker"
                && source.recipient == "/root"
