...
#[tokio::test]
async fn thread_rollback_rejects_zero_turns() -> Result<()> {
    let server = create_mock_responses_server_repeating_assistant("Done").await;
    let codex_home = TempDir::new()?;
    MockResponsesConfig::new(&server.uri()).write(codex_home.path())?;
    ...
    .await??;
    assert_eq!(rollback_err.error.code, -32600);
    assert_eq!(rollback_err.error.message, "num_turns must be >= 1");

    Ok(())
}

#[tokio::test]
async fn thread_rollback_rejects_an_active_turn() -> Result<()> {
    ...
    .await??;
    let thread_read: codex_app_server_protocol::ThreadReadResponse = to_response(read_resp)?;
    assert_eq!(thread_read.thread.turns.len(), 1);
