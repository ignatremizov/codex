                ...
                async move {
                    let config = match self.load_latest_config(Some(cwd.clone())).await {
                        ...
                        Err(error) => {
                            ...
                            // A missing instructions file may prevent rebuilding Config,
                            // but fallback must not bypass changed or unreadable policy.
                            let fallback_matches_policy = if fallback_matches_cwd {
                                self.resolve_cwd_config(&cwd).await.is_ok_and(|(_, layers)| {
                                    layers.requirements_toml()
                                        == fallback_config.config_layer_stack.requirements_toml()
                                        && layers.effective_config()
                                            == fallback_config.config_layer_stack.effective_config()
                                })
                            } else {
                                false
                            };
                            if fallback_matches_policy {
                                warn!(
                                    error = %error.message,
