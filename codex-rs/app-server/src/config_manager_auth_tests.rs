...
async fn runtime_auth_selection_is_preserved_across_config_loads() -> std::io::Result<()> {
    ...
    let reloaded = manager.load_latest_config_for_thread(&initial).await?;
    assert_eq!(reloaded.auth_config(), initial.auth_config());

    let fallback = manager.load_default_config().await?;
    assert_eq!(fallback.auth_config(), initial.auth_config());
    Ok(())
}
