...
#[test_case(ThreadHistoryMode::Legacy, false, true; "remote_v2_without_environment_context")]
#[test_case(ThreadHistoryMode::Paginated, false, true; "remote_v2_paginated_without_environment_context")]
...
async fn durable_aliases_hydrate_sampling_and_compaction_without_rewriting_history(
    ...
    let requests = responses::mount_sse_sequence(
        ...
        .map(|(index, message)| {
            ...
            if remote_compaction && message == "compact summary" {
                responses::sse(vec![
                    serde_json::json!({
                        "type": "response.output_item.done",
                        "item": {
                            "type": "compaction",
                            "encrypted_content": "identity-compaction",
                        },
                    }),
    ...
    if remote_compaction {
        let request = requests.requests()[3].clone();
        assert_eq!(identities(&request), expected);
        assert!(
            request.input().contains(&projected),
            "remote compaction request should use projected agent references",
        );
    } else {
