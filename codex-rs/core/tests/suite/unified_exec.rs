...
async fn unified_exec_emits_output_delta_for_exec_command() -> Result<()> {
    ...
    let end_event = loop {
        ...
        match event {
            EventMsg::ExecCommandOutputDelta(event) if event.call_id == call_id => {
                delta_text.push_str(&String::from_utf8_lossy(&event.chunk));
            }
    ...
    assert!(
        delta_text.contains("HELLO-UEXEC"),
        "delta chunk should arrive before command completion: {delta_text:?}",
    );
    assert!(
        end_event.stdout.contains("HELLO-UEXEC"),
        "end event missing expected text: {:?}",
        end_event.stdout,
    );
...
async fn unified_exec_terminal_interaction_captures_delayed_output() -> Result<()> {
    ...
    let open_args = json!({
        "cmd": "read -r -n 1 _; echo MARKER1; read -r -n 1 _; echo MARKER2; read -r -n 1 _",
        "yield_time_ms": 10,
        "tty": true,
    });

    // Write stdin three times to release both delayed markers and then exit.
...
async fn unified_exec_emits_one_begin_and_one_end_event() -> Result<()> {
    ...
    let [poll_started] = terminal_interactions.as_slice() else {
        panic!("expected one terminal interaction for the empty poll: {terminal_interactions:?}");
    };
    ...
    assert_command(&open_event.command, "-lc", "sleep 6");
