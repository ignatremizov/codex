...
async fn peer_completion_has_one_durable_root_row_without_implicit_model_delivery(
    ...
    let target_turn_id = target_request.body_json()["client_metadata"]["turn_id"]
        .as_str()
        .ok_or_else(|| anyhow::anyhow!("target request has no turn identity"))?
        .to_owned();
    match observation {
    ...
    let rows = history
        ...
        .collect::<Vec<_>>();
    if matches!(observation, RootObservation::Unobserved) {
        assert!(
            history.segments.iter().flatten().all(|item| !matches!(
                item,
                RolloutItem::AgentResponseObservation(observation)
                    if observation.target_thread_id == target_id
                        && observation.target_turn_id.as_deref() == Some(target_turn_id.as_str())
            )),
            "a durable hidden conclusion does not install canonical observation authority",
        );
    }
    assert_eq!(
        serde_json::to_value(rows)?,
        serde_json::to_value(vec![TurnItem::AgentMessage(completion)])?,
        "canonical replay has exactly the acknowledged terminal row",
    );
