...
async fn child_selection_captures_owning_root_and_fences_membership() {
    ...
    let mut new_membership = Box::pin(manager.state.acquire_live_agent_lifecycle(child_id));
...
async fn unloaded_parent_does_not_hide_loaded_descendants() {
    ...
    root.thread
        ...
        .expect("stop root");
    manager
        .remove_thread_if_matches(&root.thread_id, &root.thread)
        .await
        .expect("remove root");
    let mut subtree = manager
...
async fn failed_subtree_drain_retains_all_instances_until_retry() {
    ...
    // Accepted delivery owns a capability and can still use the raw lifecycle boundary.
    drop(
        manager
            .state
