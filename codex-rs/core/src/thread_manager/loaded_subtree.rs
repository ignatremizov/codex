...
pub(super) enum ThreadRemovalAuthority {
    Ordinary,
    DurableUnload,
}
...
impl LoadedSubtree {
    ...
    #[expect(
        ...
        reason = "seal exact pending and live instances atomically against ordinary removal"
    )]
    pub async fn shutdown_and_remove(&mut self) -> CodexResult<Vec<ThreadId>> {
        {
            let pending = self.state.pending_threads.write().await;
        ...
        for thread in &self.threads {
            if self
                .state
                .thread_instance_is_current_or_pending(thread)
                .await
                && self
                    .control
    ...
    pub async fn prepare_subtree_unload(&self, thread_id: ThreadId) -> CodexResult<LoadedSubtree> {
        ...
        loop {
            ...
            for id in &ordered_ids {
                if let Ok(thread) = self.state.get_thread_including_pending(*id).await {
                    thread_ids.push(*id);
                    threads.push(thread);
                }
            }
...
impl ThreadManagerState {
    /// Fence membership changes without preventing accepted completion delivery.
    pub(crate) async fn acquire_agent_membership_lifecycle(
        &self,
        thread_id: ThreadId,
    ) -> CodexResult<OwnedMutexGuard<()>> {
        let guard = self.acquire_live_agent_lifecycle(thread_id).await?;
        self.get_thread_including_pending(thread_id)
            .await?
    ...
    pub(crate) async fn retain_failed_spawn_cleanup(
        ...
        let id = thread.session.thread_id();
        let mut pending = self.pending_threads.write().await;
    ...
    async fn spawn_ancestry(&self, thread_id: ThreadId) -> CodexResult<Vec<ThreadId>> {
        ...
        loop {
            ...
            let parent = match persisted_parent {
                ...
                None => {
                    let source = match self.get_thread_including_pending(current).await {
