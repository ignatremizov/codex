...
fn build_v2_compacted_history(
    ...
    let current_omission_text = compacted_image_omission_text(&current).map(str::to_owned);
    inherited = inherited
        .into_iter()
    ...
    current = current
        ...
        .collect();
    if compacted_image_omission_text(&current).is_none()
        && let Some(omission_text) = current_omission_text.as_ref()
    {
        if let Some(ResponseItem::Message { content, .. }) = current.last_mut() {
            ...
            current.push(standalone_compacted_image_omission_message(
                omission_text.clone(),
            ));
        }
    }
    ...
    if compacted_image_omission_text(&retained).is_none()
        && let Some(omission_text) = current_omission_text
    {
        retained.push(standalone_compacted_image_omission_message(omission_text));
