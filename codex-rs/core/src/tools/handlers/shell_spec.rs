...
pub(crate) fn create_exec_command_tool_with_environment_id(
    ...
    let yield_time_ms_description = "Initial wait for exec_command before returning a session ID \
        ...
        range is 250-30000 ms (10000-30000 ms when Codex runs on Windows). Later \
...
pub fn create_write_stdin_tool(default_yield_time_ms: u64) -> ToolSpec {
    let properties = BTreeMap::from([
        ...
        (
            ...
            JsonSchema::number(Some(format!(
                "Maximum wait in uint64 ms, independent of exec_command's initial wait. Omit for configured default {default_yield_time_ms} ms. Empty polls wait at least 5000 ms and are uncapped unless subject to a configured background-poll cap; non-empty writes wait at least 250 ms and are uncapped."
            ))),
        ),
        (
            "max_output_tokens".to_string(),
