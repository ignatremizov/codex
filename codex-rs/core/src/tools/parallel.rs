                ...
                _ = cancellation_token.cancelled() => {
                        ...
                        || (!wait_for_runtime_cancellation && dispatch_handle.is_finished())
                    {
                        ...
                        if wait_for_runtime_cancellation {
                            ...
                            // Cancellation owns the terminal lifecycle event. Keep the handler
                            // alive to finish runtime and nested approval teardown without delaying
                            // the canonical abort result needed to close the model-visible call.
                            drop(tokio::spawn(
                                async move {
                                        ...
                                        && !err.is_cancelled()
                                    {
                                        tracing::warn!(
                                            ?err,
                                            "cancelled tool runtime cleanup task failed"
                                        );
                                    }
                                }
                                .in_current_span(),
