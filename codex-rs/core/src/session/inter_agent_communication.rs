...
impl Session {
    ...
    pub(crate) async fn record_inter_agent_communication(
        ...
        let response_delivery_rollout_suffix = response_delivery_id
            .as_ref()
            .and_then(|id| self.registered_communication_delivery_rollout_suffix(id));
        ...
        let completion_context_response_item_id = match completion_commit {
            CompletionCommunicationCommit::Ordinary => {
                ...
                let persisted_delivery = if response_delivery_rollout_suffix.is_some() {
                ...
                if let Some(response_delivery_rollout_suffix) =
                    response_delivery_rollout_suffix.as_ref()
                {
                    // The committed observation snapshot is the durable delivery receipt. A
                    // response item can predate that receipt (notably completion context), so a
                    // retry may still need to append the suffix without duplicating the item.
                    ...
                    if !delivery_committed {
                        ...
                        rollout_items.extend(response_delivery_rollout_suffix.iter().cloned());
                ...
                let response_already_recorded = {
                    ...
                    let already_recorded = response_delivery_rollout_suffix.is_some()
                ...
                if response_delivery_rollout_suffix.is_some()
        ...
        let join_result = tokio::spawn(async move {
            let result: Result<(), ThreadStoreError> = async {
                ...
                let persisted_delivery = match response_delivery_id.as_ref() {
                    Some(response_item_id) if response_delivery_rollout_suffix.is_some() => Some(
                ...
                if let Some(persisted_completion_item) = persisted_completion_item {
                    ...
                    {
                        live_thread
                            .append_items_and_flush_canonical(response_delivery_rollout_suffix)
                            .await?;
                    }
                } else {
                    let response_item = items[0].clone();
