...
use super::mailbox_publication::MailboxPublicationOutcome;
    ...
    pub(crate) async fn commit_mailbox_consumption(
        ...
        let accepted = self
            ...
            .ok_or_else(|| ThreadStoreError::Conflict {
                message: "mailbox receiver is closing".to_string(),
            })?;
        let session = Arc::clone(self);
        let model_info = model_info.clone();
        let commit = async move {
            let _accepted = accepted;
            let mut publication = MailboxPublicationOutcome {
            ...
            let outcome = async {
                ...
                // Drop before the barrier on every error or panic, fencing another publisher
                // before it can acquire the same history/settings boundary.
                let mut durable_outcome = MailboxPublicationOutcome {
                    session: Arc::clone(&session),
                    finished: false,
                };
                let live =
                ...
                if history.iter().enumerate().any(|(index, item)| {
                ...
                session
                    .insert_mailbox_context(&turn, &model_info, result)
                    .await?;
                ...
                for (member, delivery) in recovered.claim.messages.iter().zip(recovered.deliveries)
                {
                    ...
                    let context = if let Some(context) = existing_context {
                        ...
                        let (prepared, _) = session
                            ...
                            .await;
                        let context =
                            ResponseItemEnvelope::new(prepared.first().cloned().ok_or_else(
                                || ThreadStoreError::Internal {
                                    message: "mailbox preparation produced no context".to_string(),
                                },
                            )?);
                    ...
                    if !append.is_empty() {
                        live.append_completion_items_and_flush_canonical(&append)
                            .await?;
                    }
                    ...
                    let context_inserted = session
                        .insert_mailbox_context(&turn, &model_info, context)
                        .await?;
                    ...
                    if context_inserted || completion_was_missing {
                        let audit = completion.item.clone();
                        session
                            .tx_event
                            .send(Event {
                            ...
                            .map_err(|_| ThreadStoreError::Conflict {
                                message:
                                    "mailbox presentation stream closed after canonical commit"
                                        .to_string(),
                            })?;
                        input_audits.push(audit);
                    }
                    if complete {
            ...
            // The receiver owns durable delivery. Root copies stay live-only and never create
            // a model input, subscription, or another canonical mailbox artifact.
            if !input_audits.is_empty() {
                let control = control.clone();
                let receiver = session.thread_id;
                tokio::spawn(async move {
                    for item in input_audits {
                        if let Err(error) =
                            control.mirror_attributed_agent_input(receiver, &item).await
                        {
                            tracing::warn!(%error, "failed to mirror consumed peer input");
                        }
                    }
                });
            }
