...
#[test]
fn select_handoff_fallback_prefers_config_then_catalog_luna() {
    ...
    assert_eq!(
        select_handoff_fallback_model(Some("configured-fallback"), &available_models, &primary),
    ...
    assert_eq!(
        select_handoff_fallback_model(/*configured_model*/ None, &available_models, &primary),
        Some(HandoffModelSelection {
...
#[test]
fn select_handoff_fallback_omits_unavailable_or_duplicate_default() {
    let primary = HandoffModelSelection {
        model: DEFAULT_HANDOFF_FALLBACK_MODEL.to_string(),
        reasoning_effort: Some(ReasoningEffort::Low),
    };
    ...
    assert_eq!(
        select_handoff_fallback_model(/*configured_model*/ None, &[], &primary),
        None
    );
    assert_eq!(
        select_handoff_fallback_model(Some(DEFAULT_HANDOFF_FALLBACK_MODEL), &[], &primary),
        None
    );
}
...
async fn cancelled_handoff_decode_wins_over_invalid_helper_config() {
    ...
    let handoff = summarize_remote_compaction_handoff(
        &Arc::new(session),
    ...
    assert_eq!(handoff, RemoteCompactionHandoff::Skipped);
}
...
async fn handoff_config_is_locked_down_and_ephemeral() {
    ...
    parent_config.model_reasoning_summary = Some(ReasoningSummary::Concise);
    ...
    assert_eq!(
        helper_config.model_reasoning_summary,
