    ...
    for (idx, item) in items.iter().enumerate() {
        ...
        match item {
            RolloutItem::ResponseItem(item) if is_user_turn_boundary(item) => {
                ...
                } else if matches!(&item.item, ResponseItem::AgentMessage { .. })
...
pub(crate) fn truncate_rollout_before_nth_user_message_from_start(
    ...
    if n_from_start == usize::MAX {
        let full_range = 0..items.len();
        return exact_rollback_filtered_range(items, full_range);
    }
    ...
    exact_rollback_filtered_range(items, 0..cut_idx)
}
...
pub fn truncate_rollout_after_turn_id(
    ...
    let cut_index = items
        ...
        .unwrap_or(items.len());
    Ok(exact_rollback_filtered_range(items, 0..cut_index))
}
...
pub fn truncate_rollout_before_turn_id(
    ...
    let exact_rollback_removals = exact_rollback_removed_items(&items);
    if exact_rollback_removals[cut_index]
        || (items[cut_index + 1..]
            .iter()
...
pub(crate) fn truncate_rollout_to_last_n_fork_turns(
    ...
    let retained_range = keep_idx..items.len();
    exact_rollback_filtered_range(items, retained_range)
}

fn exact_rollback_filtered_range(
    mut items: Vec<RolloutItem>,
    range: Range<usize>,
) -> Vec<RolloutItem> {
    let removed_items = exact_rollback_removed_items(&items);
    items.truncate(range.end);
    if range.start > 0 {
        items = items.split_off(range.start);
    }

    let mut removed_items = removed_items.into_iter().skip(range.start);
    items.retain(|_| !removed_items.next().unwrap_or(true));
    items
}
