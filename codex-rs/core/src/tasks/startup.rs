...
/// Settles an admitted queue entry even when its task future is never polled or panics.
pub(super) struct TaskInputReceiptGuard {
    pub(super) session: Arc<Session>,
    pub(super) turn_id: String,
}

impl Drop for TaskInputReceiptGuard {
    fn drop(&mut self) {
        self.session.settle_queued_input_persistence(
            &self.turn_id,
            Err(codex_protocol::error::CodexErr::InvalidRequest(
                "admitted task ended without a prompt persistence receipt; do not resubmit"
                    .to_string(),
            )),
        );
    }
