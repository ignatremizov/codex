...
#[tokio::test]
async fn direct_only_namespace_does_not_override_explicit_invocation_policy() {
    let mut config = test_config().await;
    config.code_mode.direct_only_tool_namespaces = vec!["mcp__docs".to_string()];
    let tool = make_mcp_tool_with_route(
        "docs",
        "read",
        "mcp__docs",
        "read",
    ...
    for search_enabled in [false, true] {
        for implicit in [false, true] {
            let servers = HashMap::from([(
                "docs".to_string(),
                EffectiveMcpServer::configured(stdio_mcp_server_config(implicit)),
            )]);
            ...
            assert_eq!(
                (exposure.direct_tools, exposure.deferred_tools),
                if implicit {
                    (tools.clone(), None)
                } else {
                    (HashMap::new(), Some(tools.clone()))
                },
            );
        }
    }
...
async fn excludes_tools_hidden_from_model_exposure() {
    let config = test_config().await;
    let visible_tool = make_mcp_tool_with_route(
        "rmcp",
        "visible_tool",
        "mcp__rmcp",
        "visible_tool",
...
async fn app_tool_registration_uses_trusted_catalog_metadata_and_preserves_source_order() {
    let config = test_config().await;
    let app_tool = make_mcp_tool_with_route(
        CODEX_APPS_MCP_SERVER_NAME,
    ...
    let missing_connector_id = make_mcp_tool_with_route(
        CODEX_APPS_MCP_SERVER_NAME,
        "unknown_tool",
    ...
    let mut synthetic_app_tool = make_mcp_tool_with_route(
        CODEX_APPS_MCP_SERVER_NAME,
    ...
    let regular_tool = make_mcp_tool_with_route(
        "rmcp",
        "regular_tool",
        "mcp__rmcp",
        "regular_tool",
...
"#,
    ...
    let enabled_tool = make_mcp_tool_with_route(
        CODEX_APPS_MCP_SERVER_NAME,
        "events/create",
        "mcp__codex_apps__calendar",
        "create",
    ...
    let disabled_tool = make_mcp_tool_with_route(
        CODEX_APPS_MCP_SERVER_NAME,
        "events/list",
        "mcp__codex_apps__calendar",
        "list",
