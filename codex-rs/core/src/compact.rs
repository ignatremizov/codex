...
async fn run_compact_task_inner_impl(
    ...
    let summary_suffix = loop {
        ...
        match attempt_result {
            Ok(summary_suffix) => break summary_suffix,
...
pub(crate) fn collect_mcp_server_use_context_items(
    history_items: &[ResponseItem],
) -> Vec<ResponseItem> {
    history_items
        .iter()
        .filter(|item| is_mcp_server_use_context_item(item))
        .cloned()
        .collect()
}
...
fn is_mcp_server_use_context_item(item: &ResponseItem) -> bool {
...
fn build_compacted_history_with_limit(
    ...
    for message in &selected_messages {
        history.push(user_message_item(message.clone()));
    }
    ...
    history.push(user_message_item(summary_text));
...
fn selected_user_messages_with_limit(user_messages: &[String], max_tokens: usize) -> Vec<String> {
