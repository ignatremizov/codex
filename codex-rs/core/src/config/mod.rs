...
pub(crate) const DEFAULT_USER_SHELL_COMMAND_TIMEOUT_MS: u64 = 0;
...
pub struct Config {
    ...
    /// Temporary surfaces and, when `tui_fullscreen_transcript` is enabled, the owned transcript
    /// may use the alternate screen.
    ...
    /// User-controlled V1 directory discovery, independent of messaging and spawn permission.
    pub list_agents_enabled: bool,
...
impl Config {
    ...
    pub async fn rebuild_with_session_layers(
        ...
        Self::rebuild_with_session_layers_and_auth_file_selection(
            session_layers,
            cwd,
            refreshed_layers,
            codex_home,
    ...
    pub async fn rebuild_with_session_layers_and_auth_file_selection(
        session_layers: &ConfigLayerStack,
        cwd: PathBuf,
        refreshed_layers: &ConfigLayerStack,
        codex_home: AbsolutePathBuf,
...
impl Config {
    ...
    pub(crate) async fn load_config_with_layer_stack(
        ...
        let list_agents_enabled = cfg
            .tools
            .as_ref()
            .and_then(|tools| tools.list_agents.as_ref())
        ...
        let remote_compaction_handoff_fallback_model =
            cfg.remote_compaction_handoff_fallback_model;
    ...
    pub(crate) async fn try_read_non_empty_file(
        fs: &dyn ExecutorFileSystem,
