...
fn draining_resets_state_and_push_buffer_preserves_omissions() {
    let mut buf: HeadTailBuffer = HeadTailBuffer::new(/*max_bytes*/ 10);
    buf.push_chunk(b"0123456789");
    buf.push_chunk(b"ab");

    let drained = buf.drain();
    let mut collected: HeadTailBuffer = HeadTailBuffer::new(/*max_bytes*/ 10);
...
fn upstream_omission_stays_at_append_boundary() {
    let mut buf: HeadTailBuffer = HeadTailBuffer::new(/*max_bytes*/ 128);
    buf.push_chunk(b"before");
    buf.push_upstream_omission(/*omitted_bytes*/ 6);
    buf.push_chunk(b"after");
...
fn retained_upstream_omission_boundaries_are_bounded() {
    ...
    let mut buf: HeadTailBuffer = HeadTailBuffer::new(/*max_bytes*/ omission_count * 2);
    for _ in 0..omission_count {
        buf.push_chunk(b"x");
        buf.push_upstream_omission(/*omitted_bytes*/ 1);
    }
