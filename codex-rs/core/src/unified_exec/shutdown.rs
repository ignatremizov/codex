...
impl UnifiedExecProcessManager {
    ...
    pub(super) fn retain_process_for_shutdown(&self, process: Arc<UnifiedExecProcess>) {
        ...
        state.processes.retain(|process| {
            !process.actual_exit_confirmed() || !process.output_task_completion().is_cancelled()
        });
    ...
    pub(crate) async fn shutdown_durably(&self) -> Result<(), UnifiedExecError> {
        ...
        let drain = async {
            loop {
                ...
                for process in processes {
                    // Failed startup can retain a process before installing its streamer.
                    // No producer remains to consume those pipes after the launch drain.
                    if process.take_output_receiver().is_some()
                        && let Some(output_task) = process.output_task_abort_handle()
                    {
                        output_task.abort();
                    }
                    process.output_task_completion().cancelled().await;
                }
                let state = self
                    .shutdown
                    .state
                    .lock()
                    .unwrap_or_else(PoisonError::into_inner);
                if state.processes.iter().all(|process| {
                    process.actual_exit_confirmed()
                        && process.output_task_completion().is_cancelled()
                }) && state
                    .producers
                    .iter()
