...
async fn stopping_a_pending_submission_wins_before_its_launch_claim() {
    ...
    assert!(
        !tokio::time::timeout(std::time::Duration::from_secs(5), waiter)
            .await
            .expect("cancelled launch waiter should finish")
            .expect("launch waiter should join")
    );
    assert_eq!(
        manager
...
#[tokio::test]
async fn shutdown_releases_queued_launch_and_fences_late_registration() {
    let manager = Arc::new(UnifiedExecProcessManager::default());
    let _first = manager
        .reserve_user_shell_submission(UserShellCommandFinalDelivery::Passive)
        .await;
    ...
    let waiter = {
        ...
        tokio::spawn(async move {
            manager
                .wait_for_user_shell_launch(queued, /*queue_command*/ true, &cancellation)
                .await
        })
    };
    ...
    assert!(
        !tokio::time::timeout(std::time::Duration::from_secs(5), waiter)
            .await
            .expect("shutdown launch waiter should finish")
            .expect("launch waiter should join")
    );
    ...
    let process_id = manager
        .register_user_shell_command(
            ...
            PathUri::parse("file:///tmp").expect("test cwd"),
            UserShellCommandResponseHandling::default(),
            cancellation.clone(),
        )
        .await;
    ...
    assert_eq!(
        manager
            .retire_user_shell_command(process_id, "late-call")
            .await,
        Some(UserShellCommandRetirement::Stopped)
    );
}
