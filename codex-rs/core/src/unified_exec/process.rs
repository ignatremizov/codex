...
pub(crate) struct UnifiedExecProcess {
    ...
    // Unlike output_buffer, this bounded transcript is never drained by tool responses.
    output_transcript: OutputBuffer,
    output_stream_complete: CancellationToken,
    output_task_complete: CancellationToken,
...
impl UnifiedExecProcess {
    fn new(
        ...
        let output_transcript = Arc::new(Mutex::new(HeadTailBuffer::default()));
        let output_stream_complete = CancellationToken::new();
        let (output_tx, output_rx) = mpsc::channel(/*buffer*/ 64);
        ...
        Self {
            ...
            output_task_complete: CancellationToken::new(),
    ...
    pub(super) fn take_output_receiver(&self) -> Option<mpsc::Receiver<Vec<u8>>> {
        self.output_rx
            .lock()
    ...
    pub(super) fn output_transcript(&self) -> OutputBuffer {
        Arc::clone(&self.output_transcript)
    }
    ...
    pub(super) fn output_stream_completion(&self) -> CancellationToken {
        self.output_stream_complete.clone()
    }

    pub(super) fn output_task_completion(&self) -> CancellationToken {
        self.output_task_complete.clone()
    }

    pub(super) fn output_task_abort_handle(&self) -> Option<tokio::task::AbortHandle> {
        self.output_task.as_ref().map(JoinHandle::abort_handle)
    }
    ...
    pub(super) async fn from_spawned_before_classification(
        ...
        managed.output_task = Some(Self::spawn_local_output_task(
            ...
            managed.output_task_complete.clone(),
        ));
    ...
    fn spawn_exec_server_output_task(
        ...
        // Capture before spawning so abort-before-first-poll also acknowledges completion.
        let completion = output_task_complete.drop_guard();
        tokio::spawn(async move {
            ...
            let mut next_output_offset: u64 = 0;
            loop {
                    ...
                    || missing_sandbox_denial
                {
                    ...
                    for chunk in chunks.into_iter().filter(|chunk| chunk.seq > last_seq) {
                        record_exec_server_output_chunk(
                            &output_buffer,
                            &output_transcript,
                            &output_tx,
                            &output_notify,
...
async fn record_output_chunk(
    ...
    output_transcript.lock().await.push_chunk(chunk.clone());
    let _ = output_tx.send(chunk).await;
    output_notify.notify_waiters();
}

pub(super) async fn record_exec_server_output_chunk(
    output_buffer: &OutputBuffer,
    output_transcript: &OutputBuffer,
    output_tx: &mpsc::Sender<Vec<u8>>,
    output_notify: &Arc<Notify>,
    next_output_offset: &mut u64,
    chunk: codex_exec_server::ProcessOutputChunk,
) {
    let chunk_offset = if chunk.output_offset == 0 && *next_output_offset > 0 {
        // Older exec servers omit output_offset. Preserve their sequential
        // behavior while accounting for replay gaps from newer servers.
        *next_output_offset
    } else {
        chunk.output_offset
    };
    let omitted_bytes = chunk_offset.saturating_sub(*next_output_offset);
    if omitted_bytes > 0 {
        let omitted_bytes = usize::try_from(omitted_bytes).unwrap_or(usize::MAX);
        let omission_marker = format!(
            "\n{}\n",
            super::format_output_omission_marker(omitted_bytes)
        )
        .into_bytes();
        output_buffer
            .lock()
            .await
            .push_upstream_omission(omitted_bytes);
        output_transcript
            .lock()
            .await
            .push_upstream_omission(omitted_bytes);
        let _ = output_tx.send(omission_marker).await;
    }

    let bytes = chunk.chunk.into_inner();
    *next_output_offset = (*next_output_offset)
        .max(chunk_offset.saturating_add(u64::try_from(bytes.len()).unwrap_or(u64::MAX)));
    record_output_chunk(
        output_buffer,
        output_transcript,
        output_tx,
