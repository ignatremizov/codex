...
impl ContextualUserFragment for McpServerUseInstructions {
    fn content_kind(&self) -> ContentItemKind {
        ContentItemKind("mcp.server_use_instructions".to_string())
    }
    ...
    fn body(&self) -> String {
        ...
        let tools = &self.tool_inventory_json;
        let encoded_server_name = BASE64_STANDARD.encode(self.server_name.as_bytes());
        format!(
            "\n<server_name_b64>{encoded_server_name}</server_name_b64>\nThe user explicitly asked to use MCP server `{server_name}` in this session. This message provides the exact full discovered tool inventory as prompt context. You may use its tools directly if helpful.\nFull discovered tool inventory JSON:\n{tools}\n",
