...
pub(crate) fn standalone_compacted_image_omission_message(text: String) -> ResponseItem {
    ...
    let (content, content_kind) = annotated_compacted_image_omission(text).into_parts();
...
pub(crate) fn annotated_compacted_image_omission(text: String) -> AnnotatedContent {
    debug_assert!(is_compacted_image_omission_text(text.as_str()));
...
pub(crate) fn sanitize_compacted_media_prefix(
    ...
    for item in items.iter_mut().take(prefix_len) {
        match item {
            ResponseItem::Message { .. } => {
                ...
                content.retain(|item| !content_item_is_compacted_media(item.content()));
    ...
    match &mut items[marker_target_index] {
        item @ ResponseItem::Message { .. } => {
            ...
            let (_, omission_content) = omission.render_fragment().into_parts();
            content.insert(marker_insertion_index.min(content.len()), omission_content);
            let _ = set_annotated_content(item, content);
        }
        ResponseItem::FunctionCallOutput { output, .. }
        | ResponseItem::CustomToolCallOutput { output, .. } => {
    ...
    for item in items {
        match item {
            item @ ResponseItem::Message { .. } => {
                ...
                let mut content_items = VecDeque::from(content);
                while let Some(mut content_item) = content_items.pop_front() {
                    let wrapper_tail_len = if matches!(
                        ...
                        content_items.front().map(AnnotatedContent::content),
                        ...
                        (
                            content_items.front().map(AnnotatedContent::content),
                            content_items.get(1).map(AnnotatedContent::content),
                        ),
                        (
                    ...
                    if wrapper_tail_len == 0 {
                        let retain_item = match content_item.content_mut() {
                            ContentItem::InputText { text } => {
                ...
                let _ = set_annotated_content(item, retained);
            }
            ResponseItem::FunctionCallOutput { output, .. }
            | ResponseItem::CustomToolCallOutput { output, .. } => {
                if let Some(content) = output.content_items_mut() {
...
impl CompactedMediaInventory {
        ...
        for (index, item) in content.iter().enumerate() {
            match item {
                ContentItem::InputImage { image, .. } => {
                    ...
                    self.record_image(image, has_local_reference);
                }
        ...
        for item in content {
            match item {
                FunctionCallOutputContentItem::InputImage { image, .. } => {
                    ...
                    self.record_image(image, /*has_local_reference*/ false);
                }
                FunctionCallOutputContentItem::InputText { text } => {
    ...
    fn record_image(&mut self, image: &ImageReference, has_local_reference: bool) {
        ...
        self.sanitization.omitted_inline_media_bytes = self
            .sanitization
            .omitted_inline_media_bytes
            .saturating_add(match image {
                ImageReference::Inline { image_url } => {
                    u64::try_from(image_url.len()).unwrap_or(u64::MAX)
                }
                ImageReference::File { .. } => 0,
            });
...
fn content_item_is_compacted_media(item: &ContentItem) -> bool {
    match item {
        ...
        ContentItem::OutputText { .. } | ContentItem::InputAudio { .. } => false,
    }
}
