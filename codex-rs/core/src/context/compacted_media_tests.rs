...
fn annotated_user_message(content: Vec<ContentItem>) -> ResponseItem {
    let content_item_kinds = content
        .iter()
        .map(|item| match item {
            ContentItem::InputText { text } if is_compacted_image_omission_text(text) => {
                ContentItemKind("compaction.image_omission".to_string())
            }
            ContentItem::InputText { .. }
            | ContentItem::OutputText { .. }
            | ContentItem::InputImage { .. }
            | ContentItem::InputAudio { .. } => ContentItemKind("unknown".to_string()),
        })
        .collect();
    ResponseItem::Message {
        id: None,
...
fn sanitizes_images_with_bounded_reference_aware_text() {
    ...
    assert_eq!(
        items,
        vec![annotated_user_message(vec![
            ContentItem::InputText {
                text: "<image name=\"[Image #1]\" path=\"/tmp/image.png\">".to_string(),
            },
            ContentItem::InputText {
...
fn sanitizes_structured_tool_output_with_one_bounded_checkpoint_marker() {
    ...
    assert_eq!(
        items[1],
        annotated_user_message(vec![ContentItem::InputText {
            text: CompactedImageOmission::unavailable().render(),
        }])
    );
}
...
fn expiration_removes_text_only_model_image_placeholders_with_their_paths() {
    ...
    assert_eq!(
        items,
        vec![annotated_user_message(vec![ContentItem::InputText {
            text: "retain this instruction context".to_string(),
        }])]
    );
}
...
#[test]
fn expiration_removes_raw_image_triplets_returned_by_legacy_compaction() {
    let mut items = vec![user_message(vec![
        ContentItem::InputText {
            text: "<image name=[Image #1] path=\"/tmp/old.png\">".to_string(),
        },
