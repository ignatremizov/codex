...
use super::control::LocalAgentControl;
...
pub(crate) struct QueuedAgentTurn {
    pub(crate) id: Uuid,
    pub(crate) control: LocalAgentControl,
...
impl AgentTurnQueue {
    ...
    /// Restore only after positive evidence that SessionIo did not accept the input.
    pub(crate) fn restore_front_after_not_submitted(&self, target_thread_id: ThreadId, id: Uuid) {
        let mut state = self
            .state
            .lock()
            .unwrap_or_else(std::sync::PoisonError::into_inner);
    ...
    pub(crate) fn list_for_root(&self, session_id: SessionId) -> Vec<QueuedAgentTurnView> {
        ...
        let mut turns = state
            ...
            .filter(|turn| turn.control.bound_session_id() == Some(session_id))
    ...
    pub(crate) fn cancel(&self, session_id: SessionId, id: Uuid) -> bool {
        ...
        for queue in state.queues.values_mut() {
            if let Some(index) = queue.iter().position(|turn| {
                turn.id == id && turn.control.bound_session_id() == Some(session_id)
            }) {
                ...
                state
                    ...
                    .find_map(|(target_thread_id, in_flight)| {
                        (!in_flight.admission_started
                            ...
                            && in_flight.turn.control.bound_session_id() == Some(session_id))
                        .then_some(*target_thread_id)
                    });
