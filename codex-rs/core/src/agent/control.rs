...
impl AgentControl {
    ...
    pub(crate) async fn send_input_observing_response(
        ...
        let send_result = self
            .send_input_to_retained_thread(agent_id, &state, &thread, input, parent_turn_id)
            .await;
    ...
    async fn send_input_after_capacity_check(
        ...
        let thread = state.get_thread(agent_id).await?;
        self.send_input_to_retained_thread(agent_id, state, &thread, input, parent_turn_id)
            .await
    }
    ...
    async fn send_input_to_retained_thread(
        ...
        let send_result = state
            .send_user_input_to_thread(thread, input.into(), parent_turn_id)
            .await;
        let result = self
    ...
    async fn send_inter_agent_communication_durably(
        ...
        agent_communication_context: AgentCommunicationContext,
        parent_turn_id: Option<String>,
        committed_observations: Vec<codex_protocol::protocol::AgentResponseObservation>,
        ...
        if committed_observations.is_empty() {
            return Err(CodexErr::Fatal(
                "durable inter-agent response delivery requires committed observation state"
                    .to_string(),
            ));
        }
        let state = self.upgrade()?;
        ...
        let receipt = thread
            .session
            .register_communication_delivery(response_item_id, committed_observations);
    ...
    async fn send_inter_agent_completion_communication_durably(
        ...
        if committed_observations.is_empty() {
            return Err(CodexErr::Fatal(
                "durable completion response delivery requires committed observation state"
                    .to_string(),
            ));
        }
        let state = self.upgrade()?;
        self.ensure_execution_capacity_for_turn_start(parent_thread_id, communication.trigger_turn)
            .await?;
    ...
    async fn submit_inter_agent_communication(
        ...
        self.submit_inter_agent_communication_retaining_thread(
            ...
            communication,
            context,
            InterAgentSubmission::Ordinary { parent_turn_id },
        )
        .await
