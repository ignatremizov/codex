...
async fn explicit_live_wake_resume_retires_pending_mailbox_final_subscription() {
    ...
    let history = root
        ...
        .await
        .expect("canonical observer history");
    let recorded = history.iter().rev().find_map(|item| match item {
        RolloutItem::AgentResponseObservation(recorded)
            if recorded.observer_thread_id == root_id
                && recorded.target_thread_id == receiver
                && recorded.target_turn_id.is_none() =>
        {
            Some(recorded)
        }
        _ => None,
...
async fn unloaded_receiver_recovery_runs_after_thread_registration() {
    ...
    control.revoke_response_observations_for_child(receiver_thread.session.presentation_id());
    assert!(root.session.begin_agent_response_turn(sender_turn_id));
    let accepted = control
        .accept_mailbox_agent_input(
            sender,
    ...
    assert_eq!(
        ...
        None,
        "unloaded acceptance retains the SQL token without creating a synthetic live observer",
    );
