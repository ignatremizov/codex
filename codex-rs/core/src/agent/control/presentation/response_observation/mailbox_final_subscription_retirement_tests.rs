...
fn endpoints() -> (
    ...
    (
        ...
        SessionPresentationId::new(observer_thread, uuid::Uuid::now_v7()),
        SessionPresentationId::new(target_thread, uuid::Uuid::now_v7()),
    )
}

#[test]
fn latest_pending_token_suppresses_only_unclaimed_final_opportunities() {
    let (control, parent, child) = endpoints();
    control.install_mailbox_final_subscription(parent, child, "old");
    control.bind_mailbox_final_subscription_to_turn(parent, child, "old", "unclaimed");
    control.bind_mailbox_final_subscription_to_turn(parent, child, "old", "claimed");
    let receipt = ResponseItemId::new("amsg");
    control.prepare_final_response_observation_delivery(parent, child, "claimed", &receipt);

    control.install_mailbox_final_subscription(parent, child, "new");
    assert_eq!(
        control
            .mailbox_final_subscription_message_id(parent, child)
            .as_deref(),
        Some("new")
    );
    assert_eq!(
        control.response_observation_turn_final_response(parent, child, "unclaimed"),
        Some(FinalResponseObservation::None)
    );
    assert_eq!(
        control
            .prepare_final_response_observation_delivery(parent, child, "claimed", &receipt)
            .1,
        Some(receipt)
    );
    assert_eq!(
        control.response_observation_turn_final_response(parent, child, "unrelated"),
        None
    );

    control.bind_mailbox_final_subscription_to_turn(parent, child, "new", "exact");
    assert_eq!(
        control.response_observation_turn_final_response(parent, child, "exact"),
        Some(FinalResponseObservation::Wake)
    );
}

#[test]
fn retiring_an_old_token_cannot_remove_the_newer_opportunity() {
    let (control, parent, child) = endpoints();
    control.install_mailbox_final_subscription(parent, child, "old");
    control.bind_mailbox_final_subscription_to_turn(parent, child, "old", "old-turn");
    control.install_mailbox_final_subscription(parent, child, "new");
    control.bind_mailbox_final_subscription_to_turn(parent, child, "new", "new-turn");
    control.clear_mailbox_final_subscription(parent, child, "old", /*preserved_turn_id*/ None);
    assert_eq!(
        control
            .mailbox_final_subscription_message_id(parent, child)
            .as_deref(),
    ...
    assert_eq!(
        control
            .mailbox_final_subscription_for_turn(parent, child, "new-turn")
            .as_deref(),
        Some("new")
    );
    assert_eq!(
        control.response_observation_turn_final_response(parent, child, "new-turn"),
        Some(FinalResponseObservation::Wake)
    );
}
...
fn selected_next_turn_identity_survives_binding_before_mailbox_retirement() {
    ...
    control.bind_response_observation_started_turn_at_sequence(
        parent, child, "turn-u", /*sequence*/ 1,
    );
