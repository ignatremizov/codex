            ...
            .filter_map(|(&(recipient, sender), relationship)| {
                ...
                relationship.reply_route.map(|mode| {
                    (
                        ...
                        sender,
                        mode,
                        manager.agent_lifecycle_generation(other.thread_id),
                    )
                })
            })
            .collect();
...
impl LiveRevertMessagingSnapshot {
    pub(crate) fn control_for_resume(&self, thread_id: ThreadId) -> CodexResult<LocalAgentControl> {
        if thread_id != self.source.thread_id {
            return Err(CodexErr::InvalidRequest(
                "live revert cannot change thread identity".into(),
            ));
        }
    ...
    pub(crate) async fn restore_before_publication(
        ...
        if manager.get_thread(replacement.thread_id).await.is_ok() {
            return Err(CodexErr::InvalidRequest(
        ...
        let endpoints = self
            .directed
            .iter()
            .map(|&(recipient, sender, _, generation)| {
        ...
        for (other, generation) in endpoints {
            ...
            if other_thread.session.presentation_id() != other
                ...
                || manager.agent_lifecycle_generation(other.thread_id) != generation
            {
        ...
        }
        {
            ...
            let directed = self
                .directed
                .iter()
                .filter_map(|&(recipient, sender, mode, _)| {
            ...
            let retired = state
                .response_observation_by_observer_child
                .keys()
                .filter(|(recipient, sender)| *recipient == self.source || *sender == self.source)
                .copied()
                .collect::<Vec<_>>();
            for pair in retired {
                // End authority without erasing accepted old-incarnation receipt obligations.
...
impl Drop for LiveRevertMessagingSnapshot {
    fn drop(&mut self) {
        ...
        let original_still_running = self.source_runtime.upgrade().is_some_and(|thread| {
            thread.is_running()
                && self.control.upgrade().is_ok_and(|manager| {
                    manager.agent_lifecycle_generation_is_current(
