...
impl LocalAgentControl {
    pub(in crate::agent::control) fn install_persisted_send_mode(
        ...
        // Accepted observations and human queue reservations are separate obligations.
        // Scoped/model input still revalidates this effective setting at exact admission.
        drop(state);
            ...
            match setting.scope {
                ...
                AgentSendScope::Subtree {
                    ...
                    let presentation = if supervisor_thread_id == sender.thread_id {
                        ...
                        manager
                            .get_thread(supervisor_thread_id)
                            .await
                            .ok()
    ...
    pub(in crate::agent::control) async fn restore_agent_send_settings_locked(
        ...
        for id in manager.list_thread_ids().await {
            ...
            if self.session_id() != thread.session.services.agent_control.session_id()
