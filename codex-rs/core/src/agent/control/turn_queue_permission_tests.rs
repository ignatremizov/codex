...
async fn revoked_queued_message_is_not_admitted_and_warns_without_waking_source(revoke: Revoke) {
    ...
    LocalAgentControl::enqueue_agent_turn(
        &state,
        QueuedAgentTurn {
            id: queue_id,
    ...
    let reason = "scoped wake permission ended before target-turn admission; input not submitted";
    let warning_kind = "permission";
    assert_eq!(
    ...
    assert!(
        !control.target_message_wake_is_current(&QueuedTargetMessageWake {
            observer: target,
            target: source,
            target_turn_id: "queued-source-turn".into(),
            reservation_id,
        })
    );
