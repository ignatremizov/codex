...
fn false_media_free_certification_fails_closed() {
    ...
    let invalid_markers = [
        json!({
            ...
            "type": "compacted",
            "payload": {
                "message": "prefix still contains a model-visible file image",
                "replacement_history_media_sanitized_prefix_len": 1,
                "replacement_history": [{
                    "type": "message",
                    "role": "user",
                    "content": [{"type": "input_image", "file_id": "file-retained-image"}]
                }]
            }
        }),
        json!({
...
fn missing_canonical_rollout_recovers_checkpointless_vacuum_backup() {
    ...
    let backup_path = recovery::prepare_backup(&path).expect("prepare authorized backup");
...
fn missing_canonical_rollout_rejects_marker_invalid_vacuum_backup() {
    ...
    let error = recover_compacted_media_backup_if_needed(path.as_path())
        .expect_err("invalid marker cannot authorize recovery");
    assert!(error.to_string().contains("no valid media checkpoint"));
...
async fn physical_compressed_path_recovers_plain_vacuum_backup() {
    ...
    assert!(
        !path.exists(),
        "reading a backup must not publish canonical history"
    );
    assert!(backup_path.exists());
    assert!(
        recovery::manifest_path(&path)
...
fn relative_missing_canonical_rollout_recovers_vacuum_backup() {
    ...
    let backup_path = recovery::prepare_backup(&relative_path).expect("prepare authorized backup");
    ...
    assert_eq!(fs::read(&backup_path).expect("retained backup"), expected);
    remove_compacted_media_vacuum_backups(&relative_path).expect("clean relative artifacts");
}
