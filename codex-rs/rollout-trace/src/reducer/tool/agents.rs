...
fn inter_agent_message_fields(item: &ConversationItem) -> Option<InterAgentMessageFields> {
    ...
    if let Some(agent_message) = &item.agent_message {
        ...
        let (delivery_match_content, audit_content) = match encrypted_content {
            Some(encrypted_content) => (encrypted_content, None),
            None => {
                let plaintext_content = plaintext_content?;
                let audit_content = non_empty_edge_message_content(&plaintext_content);
                (plaintext_content, audit_content)
            }
        };
