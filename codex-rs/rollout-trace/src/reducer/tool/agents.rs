...
fn inter_agent_message_fields(item: &ConversationItem) -> Option<InterAgentMessageFields> {
    ...
    if let Some(agent_message) = &item.agent_message {
        let mut delivery_match_content = None;
        let mut audit_content = None;
        for part in &item.body.parts {
            match part {
                ConversationPart::Text { text } => {
                    audit_content = audit_content.or_else(|| non_empty_edge_message_content(text));
                    if delivery_match_content.is_none() {
                        delivery_match_content = Some(text.clone());
                    }
                }
                ConversationPart::Encoded { label, value } if label == "encrypted_content" => {
                    delivery_match_content = Some(value.clone());
                }
        ...
        let delivery_match_content = delivery_match_content?;
