...
struct InterAgentMessageFields {
    ...
    delivery_match_content: String,
    delivery_match_alternate_content: Option<String>,
...
impl TraceReducer {
            ...
            .position(|pending| {
                pending.target_thread_id == message_item.thread_id
                    ...
                    && (pending.delivery_match_content == message_item.delivery_match_content
                        || message_item.delivery_match_alternate_content.as_deref()
                            == Some(pending.delivery_match_content.as_str()))
            })
        else {
            return Ok(());
        };
    ...
    pub(in crate::reducer) fn resolve_pending_spawn_edge_fallbacks(&mut self) -> Result<()> {
        ...
        for pending in pending_edges {
            let message_content = pending_edge_message_content(&pending);
    ...
    fn inter_agent_message_item(&self, item_id: &str) -> Option<InterAgentMessageItem> {
        ...
        Some(InterAgentMessageItem {
            ...
            delivery_match_alternate_content: fields.delivery_match_alternate_content,
...
fn pending_edge_message_content(pending: &PendingAgentInteractionEdge) -> Option<String> {
    if pending.message_content_from_delivery_match {
        None
    } else {
        non_empty_edge_message_content(&pending.message_content)
    }
}
...
fn inter_agent_message_fields(item: &ConversationItem) -> Option<InterAgentMessageFields> {
    ...
    if let Some(agent_message) = &item.agent_message {
        let mut plaintext_content = None;
        let mut encrypted_content = None;
        for part in &item.body.parts {
            match part {
                ConversationPart::Text { text } => {
                    if plaintext_content.replace(text.clone()).is_some() {
                        return None;
                    }
                }
                ConversationPart::Encoded { label, value } if label == "encrypted_content" => {
                    if encrypted_content.replace(value.clone()).is_some() {
                        return None;
                    }
                }
        ...
        let (delivery_match_content, delivery_match_alternate_content, audit_content) =
            match encrypted_content {
                Some(encrypted_content) => (encrypted_content, None, None),
                None => {
                    let plaintext_content = plaintext_content?;
                    let payload = plaintext_agent_message_payload(
                        &plaintext_content,
                        &agent_message.author,
                        &agent_message.recipient,
                    );
                    let audit_content =
                        non_empty_edge_message_content(payload.unwrap_or(&plaintext_content));
                    let alternate_content = payload.map(str::to_string);
                    (plaintext_content, alternate_content, audit_content)
                }
            };
        return Some(InterAgentMessageFields {
            ...
            delivery_match_content,
            delivery_match_alternate_content,
            audit_content,
        });
    }
    ...
    if let [ConversationPart::Text { text }] = item.body.parts.as_slice() {
        ...
        let author_agent_path = communication.author.to_string();
        let recipient_agent_path = communication.recipient.to_string();
        let alternate_content = if communication.encrypted_content.is_none() {
            plaintext_agent_message_payload(
        ...
        let audit_content = non_empty_edge_message_content(
            alternate_content
                .as_deref()
        ...
        return Some(InterAgentMessageFields {
            ...
            recipient_agent_path,
            delivery_match_content,
            delivery_match_alternate_content: alternate_content,
            audit_content,
        });
    }
    ...
    Some(InterAgentMessageFields {
        ...
        recipient_agent_path,
        delivery_match_content,
        delivery_match_alternate_content: None,
        audit_content: None,
    })
}

fn plaintext_agent_message_payload<'a>(
    content: &'a str,
    author: &str,
    recipient: &str,
) -> Option<&'a str> {
