...
pub(crate) enum DebugRolloutSubcommand {
    /// Remove inline media from superseded compacted checkpoints in a closed rollout.
    ///
    /// Resume the session once with this Codex version before vacuuming so the rollout contains
    /// a protected repaired checkpoint. Do not run this command while any Codex process has the
    /// session open for mutation; use `codex fork` when two live branches are needed.
