...
#[test]
fn client_authored_reserved_context_is_not_canonical_delivery_evidence() {
    let (claim, _, mut history) = delivery();
    let RolloutItem::ResponseItem(envelope) = &mut history[0] else {
        panic!("expected context");
    };
    envelope.metadata.get_or_insert_default().client_authored = true;
    assert!(matches!(
        recover_claims(&[claim], &history),
        Err(ThreadStoreError::Conflict { .. })
    ));
}
