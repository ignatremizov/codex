...
#[tokio::test]
async fn recovery_requires_materialized_and_complete_canonical_history() {
    use std::io::Write;
    ...
    // A live writer path alone cannot prove that missing canonical history never
    // contained delivery evidence. Even a fresh receiver must be materialized.
    let ThreadStoreError::Conflict { message } = store
        .recover_mailbox_delivery(params.clone())
        .await
        .unwrap_err()
    else {
        panic!("unmaterialized history must stop recovery");
    };
