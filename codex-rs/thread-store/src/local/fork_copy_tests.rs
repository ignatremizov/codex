...
#[tokio::test]
async fn copied_lineage_uses_canonical_ghost_cleanup_before_rollback_coordinates() {
    ...
    let retained = message("retained after canonical cleanup");
    let removed = message("removed at canonical index two");
    let path = source.path().join("source.jsonl");
    let ghost = serde_json::json!({
        "timestamp": "2026-08-24T00:00:00.000Z", "ordinal": 1,
        "type": "response_item", "payload": {"type": "ghost_snapshot"}
    });
    let lines = [
        rollout_line(/*ordinal*/ 0, RolloutItem::SessionMeta(meta.clone())),
        ghost.to_string(),
        rollout_line(/*ordinal*/ 2, retained.clone()),
        rollout_line(/*ordinal*/ 3, removed),
        rollout_line(
            /*ordinal*/ 4,
            RolloutItem::EventMsg(EventMsg::ThreadRolledBack(ThreadRolledBackEvent {
                num_turns: 0,
    ...
    assert_rollout_items_eq(
        &copied.history.items,
        vec![RolloutItem::SessionMeta(meta), retained],
    );
}

#[tokio::test]
async fn flattening_never_manufactures_completion_metadata_adjacency() {
    let source = TempDir::new().expect("source");
    let root = ThreadId::default();
    let child = ThreadId::default();
    let root_path = source.path().join("root.jsonl");
    let child_path = source.path().join("child.jsonl");
    let metadata = RolloutItem::InterAgentCommunicationMetadata {
        trigger_turn: false,
    };
    let child_meta = session_meta(child);
    let retained = message("after segment boundary");
    let after_meta = message("after removed session header");
    let after_rollback = message("after removed response");
    let adjacent = message("original adjacent response");
    let root_contents = format!(
        "{}\n",
        [
            rollout_line(
                /*ordinal*/ 0,
                RolloutItem::SessionMeta(session_meta(root))
            ),
            rollout_line(/*ordinal*/ 1, metadata.clone()),
        ]
        .join("\n")
    );
    fs::write(&root_path, &root_contents).expect("root");
    let child_contents = [
        rollout_line(
            /*ordinal*/ 2,
            RolloutItem::SessionMeta(child_meta.clone()),
        ),
        rollout_line(/*ordinal*/ 3, retained.clone()),
        rollout_line(/*ordinal*/ 4, metadata.clone()),
        rollout_line(
            /*ordinal*/ 5,
            RolloutItem::SessionMeta(child_meta.clone()),
        ),
        rollout_line(/*ordinal*/ 6, after_meta.clone()),
        rollout_line(/*ordinal*/ 7, metadata.clone()),
        rollout_line(/*ordinal*/ 8, message("rolled back response")),
        rollout_line(
            /*ordinal*/ 9,
            RolloutItem::EventMsg(EventMsg::ThreadRolledBack(ThreadRolledBackEvent {
                num_turns: 0,
        ...
        rollout_line(/*ordinal*/ 11, metadata.clone()),
        rollout_line(/*ordinal*/ 12, adjacent.clone()),
    ];
...
#[tokio::test]
async fn compressed_copy_leaves_read_only_source_directory_unchanged() {
    let active_home = TempDir::new().expect("active home");
    let source_home = TempDir::new().expect("source home");
    let store = LocalThreadStore::new(test_config(active_home.path()), /*state_db*/ None);
    let meta = session_meta(ThreadId::default());
    let retained = message("compressed copied history");
    let path = source_home.path().join("source.jsonl.zst");
    let contents = format!(
        "{}\n",
        [
            rollout_line(/*ordinal*/ 0, RolloutItem::SessionMeta(meta.clone())),
            rollout_line(/*ordinal*/ 1, retained.clone()),
        ]
        .join("\n")
    );
    let compressed = zstd::stream::encode_all(contents.as_bytes(), /*level*/ 3).expect("compress");
    fs::write(&path, &compressed).expect("source");
    #[cfg(unix)]
    let original_permissions = {
        use std::os::unix::fs::PermissionsExt;
        let permissions = fs::metadata(source_home.path())
    ...
    let copied = load(
        ...
        LoadForkSourceByRolloutPathParams {
            rollout_path: path.with_extension(""),
        },
    )
    .await;
    #[cfg(unix)]
    fs::set_permissions(source_home.path(), original_permissions).expect("restore permissions");
    let copied = copied.expect("copy compressed source");
    assert_rollout_items_eq(
        &copied.history.items,
        vec![RolloutItem::SessionMeta(meta), retained],
    );
    assert_eq!(fs::read(&path).expect("unchanged source"), compressed);
    let names = fs::read_dir(source_home.path())
        .expect("directory")
        .map(|entry| entry.expect("entry").file_name())
        .collect::<Vec<_>>();
    assert_eq!(names, vec![std::ffi::OsString::from("source.jsonl.zst")]);
}
...
async fn copied_lineage_reads_backup_only_ancestor_without_mutating_source() {
    ...
    let source_day = source_home
        ...
        .join("2026/08/24");
    fs::create_dir_all(&source_day).expect("create source sessions");
    let root_id = ThreadId::from_string("01900000-0000-7000-8000-000000000001").expect("root id");
    let child_id = ThreadId::default();
    let root_path = source_day.join(format!("rollout-2026-08-24T00-00-00-{root_id}.jsonl"));
    // A fixed source byte stream lets the fixture exercise the production SHA-256 journal
    // contract without adding test-only hashing dependencies or weaker backup authority.
    let root_contents = concat!(
        r#"{"timestamp":"2026-08-24T00:00:00.000Z","ordinal":0,"type":"session_meta","payload":{"session_id":"01900000-0000-7000-8000-000000000001","id":"01900000-0000-7000-8000-000000000001","timestamp":"","cwd":"","originator":"","cli_version":"","history_mode":"paginated"}}"#,
        "\n",
        r#"{"timestamp":"2026-08-24T00:00:00.000Z","ordinal":1,"type":"compacted","payload":{"message":"recoverable checkpoint","replacement_history":[{"type":"message","role":"user","content":[{"type":"input_image","image_url":"data:image/png;base64,recoverable"}]}]}}"#,
        "\n",
    );
    let compacted =
        codex_rollout::parse_rollout_line(root_contents.lines().nth(1).expect("checkpoint"))
    ...
    fs::write(&backup_path, root_contents).expect("write root recovery backup");
    let manifest_path = root_path.with_file_name(format!(".{root_file_name}.media-vacuum.json"));
    let manifest = serde_json::to_vec(&serde_json::json!({
        "version": 1,
        "canonical": root_file_name,
        "backup": backup_path.file_name().and_then(|name| name.to_str()).expect("backup filename"),
        "format": "plain_jsonl",
        "source_bytes": root_contents.len(),
        "source_sha256": "caf2b32e8e796bc728bbb4c8688383f468f6a0bd413f9b5e66ae90e465f5f7a4",
        "thread_id": root_id,
    }))
    .expect("manifest");
    fs::write(&manifest_path, &manifest).expect("write recovery manifest");
    ...
    assert_eq!(
        fs::read(&manifest_path).expect("retained manifest"),
        manifest
    );
    let leaf_source = load(
        &store,
        LoadForkSourceByRolloutPathParams {
    ...
    .await
    .expect("authorized backup-only leaf");
    let root_meta =
        codex_rollout::parse_rollout_line(root_contents.lines().next().expect("metadata"))
            .expect("canonical metadata")
            .item;
    assert_rollout_items_eq(&leaf_source.history.items, vec![root_meta, compacted]);
    fs::remove_file(manifest_path).expect("remove authority");
    assert!(
        load(
            &store,
            LoadForkSourceByRolloutPathParams {
