...
pub(super) fn drop_last_n_user_turns(
    history: &mut Vec<codex_history::ResponseItemEnvelope>,
    num_turns: u32,
) {
    if num_turns == 0 {
        return;
    ...
    let user_positions = history
        ...
        .filter_map(|(index, item)| {
            (codex_history::persistent_agent_reply_route_source(item).is_none()
                && counts_as_boundary(&item.item))
            .then_some(index)
        })
        .collect::<Vec<_>>();
    ...
    while cut_index > first_turn_index
        && codex_history::persistent_agent_reply_route_source(&history[cut_index - 1]).is_none()
        && is_pre_turn_context_update(&history[cut_index - 1].item)
    {
        cut_index -= 1;
    }
