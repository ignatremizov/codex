...
fn user_agent_task_context_survives_rollback_of_surrounding_turn() {
    ...
    let RolloutItem::ResponseItem(task_item) = &task.item else {
        unreachable!()
    };
    ...
    let metadata = line(RolloutItem::InterAgentCommunicationMetadata {
        trigger_turn: false,
    });
    let lines = [
        user.clone(),
    ...
    assert!(
        plan.apply(4, lines[4].clone())
            .expect("apply rollback marker")
            .is_none()
    );
    let late = [user, metadata, task, rollback, observation];
    let plan = prepare_plan(&late);
    assert!(plan.apply(2, late[2].clone()).unwrap().is_none());
    assert!(plan.apply(4, late[4].clone()).unwrap().is_some());
}
...
#[test]
fn adjacent_observation_preserves_committed_response_in_rewritten_compaction() {
    ...
    let lines = [metadata, response, observation, compaction, rollback];
    let plan = prepare_plan(&lines);
    let replacement_history = lines
        .into_iter()
        .enumerate()
...
fn prepare_plan(lines: &[RolloutLine]) -> RollbackPlan {
    let mut provenance = super::super::context_provenance::ContextProvenance::default();
    for line in lines {
