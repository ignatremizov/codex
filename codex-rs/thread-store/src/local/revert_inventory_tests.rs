...
async fn revert_reconciles_inventory_without_blocking_unrecorded_preparations() {
    for evidence in ["absent", "recorded", "wrong-turn", "client-authored"] {
        ...
        if evidence != "absent" {
            ...
            if evidence == "client-authored" {
                context.metadata.get_or_insert_default().client_authored = true;
            }
        ...
        let current = state
            ...
            .unwrap()
            .rollout_path;
        if matches!(evidence, "wrong-turn" | "client-authored") {
