...
pub(super) async fn load(
    ...
    let requested_path = if params.rollout_path.is_relative() {
        store.config.codex_home.join(params.rollout_path)
    } else {
        params.rollout_path
    };
    ...
    // Resolve source-home symlinks even when only an authorized preimage remains. The reader
    // below decides whether a missing canonical file has journal-qualified backup authority.
    let rollout_path = match std::fs::canonicalize(&requested_path) {
        Ok(path) => path,
            ...
            let parent =
                requested_path
                    ...
                    .ok_or_else(|| ThreadStoreError::InvalidRequest {
                        message: "fork source has no parent directory".to_string(),
                    })?;
            let name =
                requested_path
                    .file_name()
                    .ok_or_else(|| ThreadStoreError::InvalidRequest {
                        message: "fork source has no file name".to_string(),
                    })?;
            std::fs::canonicalize(parent)
                .map_err(|error| lineage_read_error(parent, error))?
                .join(name)
        }
        Err(error) => return Err(lineage_read_error(&requested_path, error)),
    };
        ...
        .is_ok_and(|metadata| !metadata.is_file())
    {
        return Err(ThreadStoreError::InvalidRequest {
            message: format!(
                "fork source {} is not a regular file",
...
async fn load_segment_from_snapshot(
    ...
    loop {
        ...
        let Some(line) = codex_rollout::decode_canonical_rollout_line(value).map_err(|err| {
            ...
            // Retired ghost snapshots consume physical ordinals but not canonical rollback
            // coordinates, matching the shared resume/artifact decoder.
            continue;
        };
        if ordinal >= segment.start_ordinal() {
    ...
    let removed = codex_rollout::exact_rollback_removed_items(&items);
    let orphan_metadata = items
        .iter()
        .enumerate()
        .map(|(index, item)| {
            matches!(item, RolloutItem::InterAgentCommunicationMetadata { .. })
                && (removed.get(index + 1).copied().unwrap_or(true)
                    || !matches!(items.get(index + 1), Some(RolloutItem::ResponseItem(_))))
        })
        .collect::<Vec<_>>();
    Ok(items
        .into_iter()
        ...
        .filter(|((item, removed), orphan_metadata)| {
            !removed
                && !orphan_metadata
                && !matches!(
                    item,
